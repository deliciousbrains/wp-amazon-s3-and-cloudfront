	...
	function get_error_msg() {
		...
		if ( ! class_exists( 'Amazon_Web_Services' ) ) {
			...
			if ( file_exists( WP_PLUGIN_DIR . '/amazon-web-services/amazon-web-services.php' ) ) {
				...
				$activate_url = wp_nonce_url( self_admin_url( 'plugins.php?action=activate&amp;plugin=amazon-web-services/amazon-web-services.php' ), 'activate-plugin_amazon-web-services/amazon-web-services.php' );
		...
		if ( ! version_compare( $aws_plugin_version, $this->aws_plugin_version_required, '>=' ) ) {
			...
			$update_url = wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=amazon-web-services/amazon-web-services.php' ), 'upgrade-plugin_amazon-web-services/amazon-web-services.php' );
		...
		if ( ! version_compare( $as3cf_plugin_version, $as3cf_plugin_version_required, '>=' ) ) {
			...
			$update_url = wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' . $plugin_basename ), 'upgrade-plugin_' . $plugin_basename );
