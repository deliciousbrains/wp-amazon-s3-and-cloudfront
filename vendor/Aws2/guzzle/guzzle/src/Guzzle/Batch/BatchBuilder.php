...
class BatchBuilder
{
    ...
    protected static $mapping = array('request' => 'DeliciousBrains\\WP_Offload_S3\\Aws2\\Guzzle\\Batch\\BatchRequestTransfer', 'command' => 'DeliciousBrains\\WP_Offload_S3\\Aws2\\Guzzle\\Batch\\BatchCommandTransfer');
    /**
     ...
     * @return BatchBuilder
     */
    public function createBatchesWith(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Batch\BatchDivisorInterface $divisorStrategy)
    {
     ...
     * @return BatchBuilder
     */
    public function transferWith(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Batch\BatchTransferInterface $transferStrategy)
    {
    ...
    public function build()
    {
        if (!$this->transferStrategy) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException('No transfer strategy has been specified');
        }
        if (!$this->divisorStrategy) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException('No divisor strategy has been specified');
        }
        $batch = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Batch\Batch($this->transferStrategy, $this->divisorStrategy);
        if ($this->exceptionBuffering) {
            $batch = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Batch\ExceptionBufferingBatch($batch);
        }
        if ($this->afterFlush) {
            $batch = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Batch\NotifyingBatch($batch, $this->afterFlush);
        }
        if ($this->autoFlush) {
            $batch = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Batch\FlushingBatch($batch, $this->autoFlush);
        }
        if ($this->history) {
            $batch = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Batch\HistoryBatch($batch);
        }
        return $batch;
    }
}
