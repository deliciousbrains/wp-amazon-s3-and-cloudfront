 ...
 */
class CacheAdapterFactory implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\FromConfigInterface
{
    /**
    ...
    {
        if (!is_object($cache)) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Cache must be one of the known cache objects');
        }
        if ($cache instanceof CacheAdapterInterface) {
            return $cache;
        } elseif ($cache instanceof Cache) {
            return new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Cache\DoctrineCacheAdapter($cache);
        } elseif ($cache instanceof StorageInterface) {
            return new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Cache\Zf2CacheAdapter($cache);
        } else {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Unknown cache type: ' . get_class($cache));
        }
    }
    /**
     * Create a Guzzle cache adapter based on an array of options
    ...
    public static function factory($config = array())
    {
        ...
        if (!is_array($config)) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('$config must be an array');
        }
        if (!isset($config['cache.adapter']) && !isset($config['cache.provider'])) {
            $config['cache.adapter'] = 'DeliciousBrains\\WP_Offload_S3\\Aws2\\Guzzle\\Cache\\NullCacheAdapter';
            $config['cache.provider'] = null;
        } else {
            // Validate that the options are valid
            foreach (array('cache.adapter', 'cache.provider') as $required) {
                if (!isset($config[$required])) {
                    throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException("{$required} is a required CacheAdapterFactory option");
                }
                if (is_string($config[$required])) {
                    ...
                    if (!class_exists($config[$required])) {
                        throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException("{$config[$required]} is not a valid class for {$required}");
                    }
                }
            }
    ...
    private static function createObject($className, array $args = null)
    {
        try {
            ...
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException($e->getMessage(), $e->getCode(), $e);
        }
    }
}
