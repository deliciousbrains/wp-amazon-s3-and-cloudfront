 ...
 */
class Collection implements \ArrayAccess, \IteratorAggregate, \Countable, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\ToArrayInterface
{
    ...
    public static function fromConfig(array $config = array(), array $defaults = array(), array $required = array())
    {
        ...
        if ($missing = array_diff($required, array_keys($data))) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Config is missing the following keys: ' . implode(', ', $missing));
        }
    ...
    public function setPath($path, $value)
    {
        ...
        while (null !== ($key = array_shift($queue))) {
            if (!is_array($current)) {
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException("Trying to setPath {$path}, but {$key} is set and is not an array");
            } elseif (!$queue) {
                $current[$key] = $value;
