 ...
 */
class CachingEntityBody extends \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\AbstractEntityBodyDecorator
{
     ...
     * {@inheritdoc}
     */
    public function __construct(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\EntityBodyInterface $body)
    {
    ...
    public function seek($offset, $whence = SEEK_SET)
    {
        if ($whence == SEEK_SET) {
            ...
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException(__CLASS__ . ' supports only SEEK_SET and SEEK_CUR seek operations');
        }
        // You cannot skip ahead past where you've read from the remote stream
        if ($byte > $this->body->getSize()) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException("Cannot seek to byte {$byte} when the buffered stream only contains {$this->body->getSize()} bytes");
        }
    ...
    {
        throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException(__CLASS__ . ' does not support custom stream rewind functions');
    }
