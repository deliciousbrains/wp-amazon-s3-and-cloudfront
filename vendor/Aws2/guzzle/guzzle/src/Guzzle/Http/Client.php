...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Parser\ParserRegistry;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Parser\UriTemplate\UriTemplateInterface;
...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestFactoryInterface;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlMultiInterface;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlMultiProxy;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlHandle;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlVersion;
/**
 * HTTP client
 */
class Client extends \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\AbstractHasDispatcher implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\ClientInterface
{
    ...
    const DISABLE_REDIRECTS = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\RedirectPlugin::DISABLE;
    ...
    public function __construct($baseUrl = '', $config = null)
    {
        if (!extension_loaded('curl')) {
            ...
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException('The PHP cURL extension must be installed to use Guzzle.');
            // @codeCoverageIgnoreEnd
        }
        $this->setConfig($config ?: new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Collection());
        $this->initSsl();
        ...
        if (!$this->config[self::DISABLE_REDIRECTS]) {
            $this->addSubscriber(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\RedirectPlugin());
        }
    }
    public final function setConfig($config)
    {
        if ($config instanceof Collection) {
            ...
            $this->config = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Collection($config);
        } else {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Config must be an array or Collection');
        }
        return $this;
    }
    ...
    public final function setSslVerification($certificateAuthority = true, $verifyPeer = true, $verifyHost = 2)
    {
        ...
        if ($certificateAuthority === true) {
            ...
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('verifyPeer must be 1, 0 or boolean');
        } elseif ($verifyHost !== 0 && $verifyHost !== 1 && $verifyHost !== 2) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('verifyHost must be 0, 1 or 2');
        } else {
            ...
            if (is_file($certificateAuthority)) {
                ...
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException('Invalid option passed to ' . self::SSL_CERT_AUTHORITY . ': ' . $certificateAuthority);
            }
        }
    ...
    public function createRequest($method = 'GET', $uri = null, $headers = null, $body = null, array $options = array())
    {
        if (!$uri) {
            ...
            if (strpos($uri, '://')) {
                ...
                $url = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Url::factory($this->getBaseUrl())->combine($this->expandTemplate($uri, $templateVars));
            }
        }
    ...
    {
        return 'Guzzle/' . \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Version::VERSION . ' curl/' . \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlVersion::getInstance()->get('version') . ' PHP/' . PHP_VERSION;
    }
     ...
     * @return self
     */
    public function setCurlMulti(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlMultiInterface $curlMulti)
    {
    ...
    {
        if (!$this->curlMulti) {
            $this->curlMulti = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlMultiProxy(self::MAX_HANDLES, $this->getConfig('select_timeout') ?: self::DEFAULT_SELECT_TIMEOUT);
        }
        return $this->curlMulti;
    }
    public function setRequestFactory(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestFactoryInterface $factory)
    {
     ...
     * @return self
     */
    public function setUriTemplate(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Parser\UriTemplate\UriTemplateInterface $uriTemplate)
    {
    ...
    {
        if (!$this->uriTemplate) {
            $this->uriTemplate = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Parser\ParserRegistry::getInstance()->getParser('uri_template');
        }
     ...
     */
    protected function prepareRequest(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, array $options = array())
    {
        $request->setClient($this)->setEventDispatcher(clone $this->getEventDispatcher());
        if ($curl = $this->config[self::CURL_OPTIONS]) {
            $request->getCurlOptions()->overwriteWith(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlHandle::parseCurlConfig($curl));
        }
        if ($params = $this->config[self::REQUEST_PARAMS]) {
            \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Version::warn('request.params is deprecated. Use request.options to add default request options.');
        ...
        if ($defaults = $this->config[self::REQUEST_OPTIONS]) {
            $this->requestFactory->applyOptions($request, $defaults, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestFactoryInterface::OPTIONS_AS_DEFAULTS);
        }
        if ($options) {
    ...
    {
        \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use the request.options array to retrieve default request options');
    ...
    {
        \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Version::warn(__METHOD__ . ' is deprecated. Use the request.options array to specify default request options');
        if ($headers instanceof Collection) {
            $this->defaultHeaders = $headers;
        } elseif (is_array($headers)) {
            $this->defaultHeaders = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Collection($headers);
        } else {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Headers must be an array or Collection');
        }
        return $this;
    }
    /**
