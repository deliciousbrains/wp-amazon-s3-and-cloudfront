...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\EntityEnclosingRequest;
...
class CurlHandle
{
     ...
     */
    public static function factory(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        $requestCurlOptions = $request->getCurlOptions();
        $mediator = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\RequestMediator($request, $requestCurlOptions->get('emit_io'));
        ...
        if ($requestCurlOptions->get('debug')) {
            ...
            if (false === $curlOptions[CURLOPT_STDERR]) {
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException('Unable to create a stream for CURLOPT_STDERR');
            }
    ...
    public function __construct($handle, $options)
    {
        if (!is_resource($handle)) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Invalid handle provided');
        }
        if (is_array($options)) {
            $this->options = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Collection($options);
        } elseif ($options instanceof Collection) {
            $this->options = $options;
        } else {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Expected array or Collection');
        }
        $this->handle = $handle;
    }
    /**
    ...
    {
        return \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Url::factory($this->options->get(CURLOPT_URL));
    }
    /**
     ...
     */
    public function updateRequestFromTransfer(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($headers) {
            $parsed = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Parser\ParserRegistry::getInstance()->getParser('message')->parseRequest($headers);
