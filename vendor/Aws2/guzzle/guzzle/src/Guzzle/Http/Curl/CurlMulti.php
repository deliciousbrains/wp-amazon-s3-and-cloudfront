...
namespace DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl;
...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\RequestException;
/**
 * Send {@see RequestInterface} objects in parallel using curl_multi
 */
class CurlMulti extends \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\AbstractHasDispatcher implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlMultiInterface
{
    ...
    public function __construct($selectTimeout = 1.0)
    {
        ...
        if ($this->multiHandle === false) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\CurlException('Unable to create multi handle');
        }
    ...
    public function add(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
    ...
    public function remove(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
    ...
    {
        $multiException = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\MultiTransferException('Errors during multi transfer');
     ...
     */
    protected function beforeSend(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        try {
            $state = $request->setState(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface::STATE_TRANSFER);
            if ($state == \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface::STATE_TRANSFER) {
                $this->addHandle($request);
            } else {
                // Requests might decide they don't need to be sent just before
                // transfer (e.g. CachePlugin)
                $this->remove($request);
                if ($state == \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface::STATE_COMPLETE) {
    ...
    private function addHandle(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
     ...
     * @return CurlHandle
     */
    protected function createCurlHandle(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        $wrapper = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlHandle::factory($request);
    ...
    {
        $event = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event(array('curl_multi' => $this));
        while ($this->requests) {
     ...
     */
    protected function removeErroredRequest(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, \Exception $e = null)
    {
     ...
     */
    protected function processResponse(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlHandle $handle, array $curl)
    {
        ...
        if (!$curlException) {
            if ($this->validateResponseWasSet($request)) {
                $state = $request->setState(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface::STATE_COMPLETE, array('handle' => $handle));
        ...
        $state = $request->setState(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface::STATE_ERROR, array('exception' => $curlException));
        // Allow things to ignore the error if possible
        if ($state != \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface::STATE_TRANSFER) {
            $this->remove($request);
        }
        // The error was not handled, so fail
        if ($state == \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface::STATE_ERROR) {
     ...
     */
    protected function removeHandle(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
     ...
     */
    private function isCurlException(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlHandle $handle, array $curl)
    {
        if (CURLM_OK == $curl['result'] || CURLM_CALL_MULTI_PERFORM == $curl['result']) {
            return false;
        }
        $handle->setErrorNo($curl['result']);
        $e = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\CurlException(sprintf('[curl] %s: %s [url] %s', $handle->getErrorNo(), $handle->getError(), $handle->getUrl()));
    ...
    private function checkCurlResult($code)
    {
        if ($code != CURLM_OK && $code != CURLM_CALL_MULTI_PERFORM) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\CurlException(isset($this->multiErrors[$code]) ? "cURL error: {$code} ({$this->multiErrors[$code][0]}): cURL message: {$this->multiErrors[$code][1]}" : 'Unexpected cURL error: ' . $code);
        }
    }
    /**
     * @link https://github.com/guzzle/guzzle/issues/710
     */
    private function validateResponseWasSet(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if (!$body) {
            $rex = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\RequestException('No response was received for a request with no body. This' . ' could mean that you are saturating your network.');
            ...
            $rex = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\RequestException('The connection was unexpectedly closed. The request would' . ' have been retried, but attempting to rewind the' . ' request body failed.');
            $rex->setRequest($request);
            $this->removeErroredRequest($request, $rex);
        } else {
            $this->remove($request);
