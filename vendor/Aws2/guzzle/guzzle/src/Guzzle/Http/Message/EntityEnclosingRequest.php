...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\RedirectPlugin;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\RequestException;
/**
 * HTTP request that sends an entity-body in the request message (POST, PUT, PATCH, DELETE)
 */
class EntityEnclosingRequest extends \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Request implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\EntityEnclosingRequestInterface
{
    ...
    {
        $this->postFields = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryString();
    ...
    public function setBody($body, $contentType = null)
    {
        ...
        if ($size !== null && $size !== false) {
            ...
            if ('1.1' == $this->protocolVersion) {
                ...
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\RequestException('Cannot determine Content-Length and cannot use chunked Transfer-Encoding when using HTTP/1.0');
            }
        }
        return $this;
    }
    ...
    {
        $this->getParams()->set(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\RedirectPlugin::STRICT_REDIRECTS, $strict);
        if ($maxRedirects == 0) {
            $this->getParams()->set(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\RedirectPlugin::DISABLE, true);
        } else {
            $this->getParams()->set(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\RedirectPlugin::MAX_REDIRECTS, $maxRedirects);
        }
        return $this;
    }
    ...
    public function addPostFile($field, $filename = null, $contentType = null, $postname = null)
    {
        ...
        if ($field instanceof PostFileInterface) {
            ...
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\RequestException('The path to a file must be a string');
        } elseif (!empty($filename)) {
            // Adding an empty file will cause cURL to error out
            $data = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\PostFile($field, $filename, $contentType, $postname);
        }
        if ($data) {
    ...
    public function addPostFiles(array $files)
    {
        foreach ($files as $key => $file) {
            if ($file instanceof PostFileInterface) {
                ...
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\RequestException('File must be a string or instance of PostFileInterface');
            }
        }
        return $this;
    }
