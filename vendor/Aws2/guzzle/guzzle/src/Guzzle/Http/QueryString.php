...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryAggregator\QueryAggregatorInterface;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryAggregator\PhpAggregator;
/**
 * Query string object to handle managing query string parameters and aggregating those parameters together as a string.
 */
class QueryString extends \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Collection
{
    ...
    public static function fromString($query)
    {
        ...
        if ($foundDuplicates && !$foundPhpStyle) {
            $q->setAggregator(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryAggregator\DuplicateAggregator());
        }
        return $q;
    }
    /**
     ...
     */
    public function setAggregator(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryAggregator\QueryAggregatorInterface $aggregator = null)
    {
        // Use the default aggregator if none was set
        if (!$aggregator) {
            if (!self::$defaultAggregator) {
                self::$defaultAggregator = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryAggregator\PhpAggregator();
            }
