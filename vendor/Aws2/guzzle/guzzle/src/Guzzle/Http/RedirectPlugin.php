...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\TooManyRedirectsException;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\CouldNotRewindStreamException;
use DeliciousBrains\WP_Offload_S3\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface;
/**
 * Plugin to implement HTTP redirects. Can redirect like a web browser or using strict RFC 2616 compliance
 */
class RedirectPlugin implements \DeliciousBrains\WP_Offload_S3\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     */
    public function cleanupRequest(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event $event)
    {
     ...
     */
    protected function getOriginalRequest(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        $original = $request;
     ...
     */
    protected function createRedirectRequest(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, $statusCode, $location, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $original)
    {
        ...
        if ($request instanceof EntityEnclosingRequestInterface && ($statusCode == 303 || !$strict && $statusCode <= 302)) {
            $redirectRequest = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestFactory::getInstance()->cloneRequestWithMethod($request, 'GET');
        } else {
        ...
        $location = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Url::factory($location);
        ...
        $redirectRequest->getEventDispatcher()->addListener('request.before_send', $func = function ($e) use(&$func, $request, $redirectRequest) {
            ...
            $e['request']->getParams()->set(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\RedirectPlugin::PARENT_REQUEST, $request);
        });
     ...
     * @return RequestInterface
     */
    protected function prepareRedirection(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $original, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response $response)
    {
     ...
     */
    protected function sendRedirectRequest(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $original, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response $response)
    {
     ...
     */
    protected function throwTooManyRedirectsException(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $original, $max)
    {
        $original->getEventDispatcher()->addListener('request.complete', $func = function ($e) use(&$func, $original, $max) {
            ...
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\TooManyRedirectsException($str);
        });
    }
}
