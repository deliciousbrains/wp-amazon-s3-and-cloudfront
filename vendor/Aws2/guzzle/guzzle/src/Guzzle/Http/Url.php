...
class Url
{
    ...
    public static function factory($url)
    {
        ...
        if (false === ($parts = parse_url($url))) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Was unable to parse malformed url: ' . $url);
        }
        $parts += $defaults;
        // Convert the query string into a QueryString object
        if ($parts['query'] || 0 !== strlen($parts['query'])) {
            $parts['query'] = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryString::fromString($parts['query']);
        }
     ...
     */
    public function __construct($scheme, $host, $username = null, $password = null, $port = null, $path = null, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryString $query = null, $fragment = null)
    {
        $this->scheme = $scheme;
        $this->host = $host;
        $this->port = $port;
        ...
        if (!$query) {
            $this->query = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryString();
        } else {
            $this->setQuery($query);
        }
        $this->setPath($path);
    ...
    public function setQuery($query)
    {
        if (is_string($query)) {
            ...
            $this->query = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryString($output);
        } elseif (is_array($query)) {
            $this->query = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryString($query);
    ...
    private function addQuery(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\QueryString $new, $strictRfc386)
    {
        if (!$strictRfc386) {
