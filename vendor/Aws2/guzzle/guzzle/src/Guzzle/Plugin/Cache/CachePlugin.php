...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Cache\CacheAdapterFactory;
...
use DeliciousBrains\WP_Offload_S3\Aws2\Doctrine\Common\Cache\ArrayCache;
 ...
 */
class CachePlugin implements \DeliciousBrains\WP_Offload_S3\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    ...
    public function __construct($options = null)
    {
        if (!is_array($options)) {
            if ($options instanceof CacheAdapterInterface) {
                $options = array('storage' => new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cache\DefaultCacheStorage($options));
            } elseif ($options instanceof CacheStorageInterface) {
                $options = array('storage' => $options);
            } elseif ($options) {
                $options = array('storage' => new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cache\DefaultCacheStorage(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Cache\CacheAdapterFactory::fromCache($options)));
            } elseif (!class_exists('DeliciousBrains\\WP_Offload_S3\\Aws2\\Doctrine\\Common\\Cache\\ArrayCache')) {
                // @codeCoverageIgnoreStart
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('No cache was provided and Doctrine is not installed');
                // @codeCoverageIgnoreEnd
            }
        }
        $this->autoPurge = isset($options['auto_purge']) ? $options['auto_purge'] : false;
        // Add a cache storage if a cache adapter was provided
        $this->storage = isset($options['storage']) ? $options['storage'] : new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cache\DefaultCacheStorage(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Cache\DoctrineCacheAdapter(new \DeliciousBrains\WP_Offload_S3\Aws2\Doctrine\Common\Cache\ArrayCache()));
        if (!isset($options['can_cache'])) {
            $this->canCache = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cache\DefaultCanCacheStrategy();
        } else {
            $this->canCache = is_callable($options['can_cache']) ? new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cache\CallbackCanCacheStrategy($options['can_cache']) : $options['can_cache'];
        }
        // Use the provided revalidation strategy or the default
        $this->revalidation = isset($options['revalidation']) ? $options['revalidation'] : new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cache\DefaultRevalidation($this->storage, $this->canCache);
    }
    ...
    public function onRequestBeforeSend(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event $event)
    {
        ...
        $request->addHeader('Via', sprintf('%s GuzzleCache/%s', $request->getProtocolVersion(), \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Version::VERSION));
        if (!$this->canCache->canCacheRequest($request)) {
            switch ($request->getMethod()) {
                case 'PURGE':
                    $this->purge($request);
                    $request->setResponse(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response(200, array(), 'purged'));
                    break;
                case 'PUT':
                case 'POST':
                case 'DELETE':
     ...
     * @return null
     */
    public function onRequestException(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event $event)
    {
     ...
     * @return bool
     */
    public function canResponseSatisfyRequest(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response $response)
    {
     ...
     * @return bool
     */
    public function canResponseSatisfyFailedRequest(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response $response)
    {
        $reqc = $request->getHeader('Cache-Control');
        $resc = $response->getHeader('Cache-Control');
     ...
     */
    protected function addResponseHeaders(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response $response)
    {
        $params = $request->getParams();
        $response->setHeader('Via', sprintf('%s GuzzleCache/%s', $request->getProtocolVersion(), \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Version::VERSION));
        ...
        if ($response->isFresh() === false) {
            $response->addHeader('Warning', sprintf('110 GuzzleCache/%s "Response is stale"', \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Version::VERSION));
            if ($params['cache.hit'] === 'error') {
                $response->addHeader('Warning', sprintf('111 GuzzleCache/%s "Revalidation failed"', \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Version::VERSION));
            }
        }
    }
}
