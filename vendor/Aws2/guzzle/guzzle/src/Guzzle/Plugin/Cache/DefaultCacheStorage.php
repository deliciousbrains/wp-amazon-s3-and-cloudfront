...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\MessageInterface;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Request;
 ...
 */
class DefaultCacheStorage implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cache\CacheStorageInterface
{
    /** @var string */
    protected $keyPrefix;
    ...
    {
        $this->cache = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Cache\CacheAdapterFactory::fromCache($cache);
    ...
    public function purge($url)
    {
        foreach (array('GET', 'HEAD', 'POST', 'PUT', 'DELETE') as $method) {
            $this->delete(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Request($method, $url));
        }
    }
    public function fetch(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($match[4] < time()) {
            ...
            $response = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response($match[2], $match[1]);
            if ($match[3]) {
     ...
     * @return string
     */
    protected function getCacheKey(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
     ...
     * @return string
     */
    protected function getBodyKey($url, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\EntityBodyInterface $body)
    {
     ...
     * @return array
     */
    private function persistHeaders(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\MessageInterface $message)
    {
