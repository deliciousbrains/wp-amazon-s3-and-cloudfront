...
namespace DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\CookieJar;

use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie;
...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Exception\InvalidCookieException;
/**
 * Cookie cookieJar that stores cookies an an array
 */
class ArrayCookieJar implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\CookieJar\CookieJarInterface, \Serializable
{
    ...
    public function remove($domain = null, $path = null, $name = null)
    {
        ...
        $this->cookies = array_filter($this->cookies, function (\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie $cookie) use($cookies) {
    ...
    {
        $this->cookies = array_filter($this->cookies, function (\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie $cookie) {
    ...
    {
        $currentTime = time();
        $this->cookies = array_filter($this->cookies, function (\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie $cookie) use($currentTime) {
    ...
    {
        return array_values(array_filter($this->cookies, function (\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie $cookie) use($domain, $path, $name, $skipDiscardable, $skipExpired) {
    ...
    public function add(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie $cookie)
    {
        // Only allow cookies with set and valid domain, name, value
        $result = $cookie->validate();
        if ($result !== true) {
            if ($this->strictMode) {
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Exception\InvalidCookieException($result);
            } else {
    ...
    public function serialize()
    {
        ...
        return json_encode(array_map(function (\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie $cookie) {
    ...
    public function unserialize($data)
    {
        ...
        if (empty($data)) {
            ...
            $this->cookies = array_map(function (array $cookie) {
                return new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie($cookie);
            }, $data);
        }
    }
    /**
    ...
    public function addCookiesFromResponse(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response $response, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request = null)
    {
        if ($cookieHeader = $response->getHeader('Set-Cookie')) {
            $parser = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Parser\ParserRegistry::getInstance()->getParser('cookie');
            foreach ($cookieHeader as $cookie) {
                if ($parsed = $request ? $parser->parseCookie($cookie, $request->getHost(), $request->getPath()) : $parser->parseCookie($cookie)) {
                    ...
                    foreach ($parsed['cookies'] as $key => $value) {
                        ...
                        $row['value'] = $value;
                        unset($row['cookies']);
                        $this->add(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie($row));
                    }
                }
            }
        }
    }
    public function getMatchingCookies(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
     ...
     */
    private function removeCookieIfEmpty(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Cookie\Cookie $cookie)
    {
