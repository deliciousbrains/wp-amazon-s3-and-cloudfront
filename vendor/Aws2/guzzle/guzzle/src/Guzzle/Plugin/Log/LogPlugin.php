...
namespace DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Log;
...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Log\MessageFormatter;
 ...
 */
class LogPlugin implements \DeliciousBrains\WP_Offload_S3\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     */
    public function __construct(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Log\LogAdapterInterface $logAdapter, $formatter = null, $wireBodies = false)
    {
        $this->logAdapter = $logAdapter;
        $this->formatter = $formatter instanceof MessageFormatter ? $formatter : new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Log\MessageFormatter($formatter);
    ...
    public static function getDebugPlugin($wireBodies = true, $stream = null)
    {
        ...
        return new self(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Log\ClosureLogAdapter(function ($m) use($stream) {
     ...
     * @param Event $event
     */
    public function onCurlRead(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event $event)
    {
     ...
     * @param Event $event
     */
    public function onCurlWrite(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event $event)
    {
    ...
    public function onRequestBeforeSend(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event $event)
    {
        if ($this->wireBodies) {
            ...
            if ($request instanceof EntityEnclosingRequestInterface && $request->getBody() && (!$request->getBody()->isSeekable() || !$request->getBody()->isReadable())) {
                ...
                $request->getParams()->set('request_wire', \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\EntityBody::factory());
            }
            if (!$request->getResponseBody()->isRepeatable()) {
                ...
                $request->getParams()->set('response_wire', \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\EntityBody::factory());
            }
        }
    }
    /**
