...
namespace DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Md5;
 ...
 */
class Md5ValidatorPlugin implements \DeliciousBrains\WP_Offload_S3\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
     ...
     */
    public function onRequestComplete(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event $event)
    {
        ...
        if ($contentMd5 !== $hash) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\UnexpectedValueException("The response entity body may have been modified over the wire.  The Content-MD5 " . "received ({$contentMd5}) did not match the calculated MD5 hash ({$hash}).");
        }
    }
}
