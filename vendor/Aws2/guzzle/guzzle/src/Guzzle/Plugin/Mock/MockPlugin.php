...
namespace DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Plugin\Mock;
 ...
 */
class MockPlugin extends \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\AbstractHasDispatcher implements \DeliciousBrains\WP_Offload_S3\Aws2\Symfony\Component\EventDispatcher\EventSubscriberInterface, \Countable
{
    ...
    {
        if (!file_exists($path)) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Unable to open mock file: ' . $path);
        }
        return \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\Response::fromMessage(file_get_contents($path));
    }
    /**
    ...
    public function addResponse($response)
    {
        if (!$response instanceof Response) {
            if (!is_string($response)) {
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Invalid response');
            }
     ...
     * @return MockPlugin
     */
    public function addException(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Exception\CurlException $e)
    {
        $this->queue[] = $e;
        return $this;
    }
     ...
     */
    public function dequeue(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface $request)
    {
        ...
        if ($item instanceof Response) {
            if ($this->readBodies && $request instanceof EntityEnclosingRequestInterface) {
                $request->getEventDispatcher()->addListener('request.sent', $f = function (\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Event $event) use(&$f) {
            ...
            $state = $request->setState(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Message\RequestInterface::STATE_ERROR, array('exception' => $item));
