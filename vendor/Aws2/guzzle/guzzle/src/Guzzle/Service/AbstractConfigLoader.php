 ...
 */
abstract class AbstractConfigLoader implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\ConfigLoaderInterface
{
    ...
    public function load($config, array $options = array())
    {
        ...
        if (is_string($config)) {
            ...
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Unknown type passed to configuration loader: ' . gettype($config));
        } else {
    ...
    protected function loadFile($filename)
    {
        ...
        switch (pathinfo($filename, PATHINFO_EXTENSION)) {
            ...
            case 'json':
                ...
                if ($json === false) {
                    $err = error_get_last();
                    throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException("Unable to open {$filename}: " . $err['message']);
                }
                ...
                if ($error = json_last_error()) {
                    ...
                    throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\RuntimeException("Error loading JSON data from {$filename}: ({$error}) - {$message}");
                }
                break;
            case 'php':
                ...
                if (!is_array($config)) {
                    throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('PHP files must return an array of configuration data');
                }
                break;
            default:
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Unknown file extension: ' . $filename);
        }
