...
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\ValidatorInterface;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\SchemaValidator;
use DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Exception\CommandException;
 ...
 */
abstract class AbstractCommand extends \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Collection implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Command\CommandInterface
{
     ...
     */
    public function __construct($parameters = array(), \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\OperationInterface $operation = null)
    {
        ...
        if (!$headers instanceof Collection) {
            $this[self::HEADERS_OPTION] = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Collection((array) $headers);
        }
    ...
    public function setOnComplete($callable)
    {
        if (!is_callable($callable)) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('The onComplete function must be callable');
        }
    ...
    {
        if (!$this->client) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Exception\CommandException('A client must be associated with the command before it can be executed.');
        }
    ...
    public function setClient(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\ClientInterface $client)
    {
        $this->client = $client;
        return $this;
    }
    ...
    {
        if (!$this->request) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Exception\CommandException('The command must be prepared before retrieving the request');
        }
        return $this->request;
    }
    ...
    public function prepare()
    {
        if (!$this->isPrepared()) {
            if (!$this->client) {
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Exception\CommandException('A client must be associated with the command before it can be prepared.');
            }
            ...
            if ($options = $this[\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Client::CURL_OPTIONS]) {
                $this->request->getCurlOptions()->overwriteWith(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Http\Curl\CurlHandle::parseCurlConfig($options));
            }
     ...
     * @return self
     */
    public function setValidator(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\ValidatorInterface $validator)
    {
    ...
    {
        return new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Operation(array('name' => get_class($this)));
    }
    /**
    ...
    {
        $this->result = $this[self::RESPONSE_PROCESSING] != self::TYPE_RAW ? \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Command\DefaultResponseParser::getInstance()->parse($this) : $this->request->getResponse();
    }
    /**
    ...
    {
        if (!$this->validator) {
            $this->validator = \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\SchemaValidator::getInstance();
        }
