 ...
 */
class CompositeFactory implements \IteratorAggregate, \Countable, \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Command\Factory\FactoryInterface
{
     ...
     * @return self
     */
    public static function getDefaultChain(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\ClientInterface $client)
    {
        $factories = array();
        if ($description = $client->getDescription()) {
            $factories[] = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Command\Factory\ServiceDescriptionFactory($description);
        }
        $factories[] = new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Command\Factory\ConcreteClassFactory($client);
     ...
     */
    public function add(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Command\Factory\FactoryInterface $factory, $before = null)
    {
        $pos = null;
        if ($before) {
