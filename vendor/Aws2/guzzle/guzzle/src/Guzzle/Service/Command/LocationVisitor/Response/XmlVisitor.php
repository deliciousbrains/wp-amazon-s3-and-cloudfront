 ...
 */
class XmlVisitor extends \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Command\LocationVisitor\Response\AbstractResponseVisitor
{
     ...
     */
    protected function recursiveProcess(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter $param, &$value)
    {
        $type = $param->getType();
        if (!is_array($value)) {
            if ($type == 'array') {
     ...
     */
    protected function processArray(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter $param, &$value)
    {
     ...
     * @param mixed     $value Value to process
     */
    protected function processObject(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter $param, &$value)
    {
     ...
     */
    protected function processXmlAttribute(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter $property, array &$value)
    {
        $sentAs = $property->getWireName();
