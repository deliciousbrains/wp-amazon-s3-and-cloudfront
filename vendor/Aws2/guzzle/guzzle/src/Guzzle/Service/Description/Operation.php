 ...
 */
class Operation implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\OperationInterface
{
    /** @var string Default command class to use when none is specified */
    const DEFAULT_COMMAND_CLASS = 'DeliciousBrains\\WP_Offload_S3\\Aws2\\Guzzle\\Service\\Command\\OperationCommand';
     ...
     */
    public function __construct(array $config = array(), \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\ServiceDescriptionInterface $description = null)
    {
        ...
        if ($this->parameters) {
            foreach ($this->parameters as $name => $param) {
                if ($param instanceof Parameter) {
                    ...
                    $param['name'] = $name;
                    $this->addParam(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter($param, $this->description));
                }
            }
        }
        if ($this->additionalParameters) {
            if ($this->additionalParameters instanceof Parameter) {
                ...
                $this->setadditionalParameters(new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter($this->additionalParameters, $this->description));
            }
        }
    }
     ...
     * @return self
     */
    public function addParam(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter $param)
    {
     ...
     * 'DeliciousBrains\\WP_Offload_S3\\Aws2\\Guzzle\\Foo\\Baz', or 'MyModelName' (to reference a model by ID).
     *
    ...
    public function setResponseType($responseType)
    {
        ...
        if (!isset($types[$responseType])) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('responseType must be one of ' . implode(', ', array_keys($types)));
        }
