...
class Parameter
{
     ...
     */
    public function __construct(array $data = array(), \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\ServiceDescriptionInterface $description = null)
    {
        if ($description) {
            if (isset($data['$ref'])) {
    ...
    public function filter($value)
    {
        ...
        if ($this->format) {
            return \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\SchemaFormatter::format($this->format, $value);
        }
    ...
    public function addFilter($filter)
    {
        if (is_array($filter)) {
            if (!isset($filter['method'])) {
                throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('A [method] value must be specified for each complex filter');
            }
        }
        if (!$this->filters) {
     ...
     * @return self
     */
    public function addProperty(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter $property)
    {
     ...
     * @return self
     */
    public function setItems(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Service\Description\Parameter $items = null)
    {
        if ($this->items = $items) {
