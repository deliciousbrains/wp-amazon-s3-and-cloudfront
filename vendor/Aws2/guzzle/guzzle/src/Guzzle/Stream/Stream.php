 ...
 */
class Stream implements \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Stream\StreamInterface
{
     ...
     */
    public static function getHash(\DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Stream\StreamInterface $stream, $algo, $rawOutput = false)
    {
        $pos = $stream->ftell();
        if (!$stream->seek(0)) {
            return false;
    ...
    public function setStream($stream, $size = null)
    {
        if (!is_resource($stream)) {
            throw new \DeliciousBrains\WP_Offload_S3\Aws2\Guzzle\Common\Exception\InvalidArgumentException('Stream must be a resource');
        }
        $this->size = $size;
        $this->stream = $stream;
        $this->rebuildCache();
