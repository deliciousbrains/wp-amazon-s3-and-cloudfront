...
class Signer
{
     ...
     */
    public const SUPPORTED_ALGORITHMS = ['SHA1' => \true, 'SHA256' => \true];
    /**
     ...
     */
    private const OPENSSL_ALGORITHM_NAMES = [\OPENSSL_ALGO_SHA1 => 'SHA1', \OPENSSL_ALGO_SHA256 => 'SHA256'];
    /**
     ...
     */
    public function __construct($keyPairId, $privateKey, $passphrase = "", string|int $algorithm = self::DEFAULT_ALGORITHM)
    {
        ...
        if (!isset(self::SUPPORTED_ALGORITHMS[$algorithm])) {
            throw new \InvalidArgumentException("Unsupported signature algorithm: {$algorithm}. Supported algorithms are: " . \implode(', ', \array_keys(self::SUPPORTED_ALGORITHMS)) . '.');
        }
    ...
    private function validateKeyType($pkHandle) : void
    {
        ...
        if ($details === \false) {
            throw new \InvalidArgumentException('Unable to read the details of the provided private key.');
        }
        ...
        if (\defined('OPENSSL_KEYTYPE_EC') && $type === \OPENSSL_KEYTYPE_EC) {
            ...
            throw new \InvalidArgumentException("Unsupported CloudFront key type: ECDSA on curve '{$curve}'. " . 'CloudFront requires ECDSA keys to be on the P-256 curve ' . '(prime256v1 / secp256r1).');
        }
        throw new \InvalidArgumentException('Unsupported CloudFront key type. CloudFront requires an RSA or ' . 'ECDSA P-256 (prime256v1) private key.');
    }
    /**
