...
namespace DeliciousBrains\WP_Offload_Media\Aws3\Aws\Configuration;
...
class ConfigurationResolver
{
    ...
    public static function resolve($key, $defaultValue, $expectedType, $config = [])
    {
        ...
        $useSharedConfig = $config['use_aws_shared_config_files'] ?? \true;
        if ($useSharedConfig !== \false) {
            $iniValue = self::ini($key, $expectedType, null, null, $config['ini_resolver_options'] ?? []);
            if ($iniValue !== null) {
                return $iniValue;
            }
        }
    ...
    public static function env($key, $expectedType = 'string')
    {
        ...
        return $expectedType ? self::convertType($envValue, $expectedType) : $envValue;
    }
    /**
    ...
    public static function ini($key, $expectedType, $profile = null, $filename = null, $options = [])
    {
        ...
        $data = @\DeliciousBrains\WP_Offload_Media\Aws3\Aws\parse_ini_file($filename, \true, \INI_SCANNER_TYPED);
        if ($data === \false) {
            return null;
        }
    ...
    private static function getProfileSection(array $data, $profile)
    {
        if ($profile === self::DEFAULT_PROFILE) {
            return $data[self::DEFAULT_PROFILE] ?? $data[self::PROFILE_PREFIX . self::DEFAULT_PROFILE] ?? null;
        }
        return $data[self::PROFILE_PREFIX . $profile] ?? $data[$profile] ?? null;
    }
    /**
     ...
     * @return mixed|null
     */
    private static function retrieveValueFromIniSubsection(array $data, $profile, $filename, $expectedType, array $options)
    {
        ...
        $subsections = \DeliciousBrains\WP_Offload_Media\Aws3\Aws\parse_ini_section_with_subsections($filename, $referencedSection);
    ...
    private static function toBool(mixed $value) : mixed
    {
        ...
        return \DeliciousBrains\WP_Offload_Media\Aws3\Aws\boolean_value($value) ?? $value;
    }
    /**
