...
use DeliciousBrains\WP_Offload_Media\Aws3\Aws\Signin\SigninClient;
use DeliciousBrains\WP_Offload_Media\Aws3\Aws\Signin\Exception\SigninException;
...
final class LoginCredentialProvider
{
        ...
        return Promise\Coroutine::of(function () {
            ...
            if ($this->shouldRefresh($credentials)) {
                try {
                    ...
                    \trigger_error('Continuing with existing token after refresh failure: ' . $e->getMessage(), \E_USER_NOTICE);
                }
            }
            (yield $credentials);
        });
    ...
    private function refresh(Credentials $currentCredentials) : \Generator
    {
        ...
        try {
            ...
            $newCredentials = self::createCredentials($refreshed[self::KEY_ACCESS_TOKEN], \time() + $refreshed[self::RESULT_EXPIRES_IN], $currentCredentials->getAccountId());
        ...
        try {
            ...
            \trigger_error('Failed to update credential cache during refresh: ' . $e->getMessage() . '. Using refreshed credentials in memory.', \E_USER_NOTICE);
        }
    ...
    {
        return $credentials->getExpiration() - \time() <= self::DEFAULT_REFRESH_THRESHOLD;
    }
    /**
    ...
    private function handleRefreshException(\Exception $e) : \Exception
    {
        if ($e instanceof SigninException) {
            \trigger_error('Failed to refresh login credentials: ' . $e->getAwsErrorMessage(), \E_USER_NOTICE);
        ...
        \trigger_error('Unexpected error refreshing login credentials: ' . $e->getMessage(), \E_USER_NOTICE);
        return $e;
    }
    /**
    ...
    private function resolveTokenLocation(string $profileName) : string
    {
        ...
        $cacheFile = $cacheDirectory . \DIRECTORY_SEPARATOR . \hash('sha256', \trim($session)) . '.json';
