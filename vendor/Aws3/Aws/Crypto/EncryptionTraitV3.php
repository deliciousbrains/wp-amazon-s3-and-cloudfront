...
trait EncryptionTraitV3
{
     ...
     * @internal
     */
    public function encrypt(StreamInterface $plaintext, AlgorithmSuite $algorithmSuite, array $options, MaterialsProviderV3 $provider, MetadataEnvelope $envelope) : AppendStream
    {
    ...
    private function encryptNonCommitingStream(StreamInterface $plaintext, array &$cipherOptions, array $keys, array $materialsDescription, string $aesName, MaterialsProviderV3 $provider, MetadataEnvelope $envelope) : AppendStream
    {
        //= ../specification/s3-encryption/encryption.md#content-encryption
    ...
    private function encryptCommitingStream(StreamInterface $plaintext, AlgorithmSuite $algorithmSuite, array &$options, array $keys, array $materialsDescription, MaterialsProviderV3 $provider, MetadataEnvelope $envelope) : AppendStream
    {
        //= ../specification/s3-encryption/encryption.md#content-encryption
        //# The generated IV or Message ID MUST be set or returned from the
        //# encryption process such that it can be included in the content metadata.
     ...
     * @internal
     */
    protected function getNonCommittingEncryptingStream(StreamInterface $plaintext, string $cek, array &$cipherOptions) : AppendStream
    {
     ...
     * @internal
     */
    protected function getCommitingEncryptionStream(StreamInterface $plaintext, string $dek, array &$cipherOptions, string $messageId, AlgorithmSuite $algorithmSuite) : array
    {
        ...
        switch ($cipherOptions['Cipher']) {
            ...
            case 'gcm':
                ...
                $encryptClass = self::$encryptClasses[$algorithmSuite->getCipherName()];
                if (!empty($cipherOptions['Aad'])) {
                    \trigger_error("'Aad' has been supplied for content encryption" . " with " . $encryptClass::getStaticAesName() . ". The" . " PHP SDK encryption client can decrypt an object" . " encrypted in this way, but other AWS SDKs may not be" . " able to.", \E_USER_NOTICE);
                }
