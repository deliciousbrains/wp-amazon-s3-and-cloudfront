...
final class BddEvaluator
{
    ...
    private const RESULT_OFFSET = 100000000;
    public function __construct(private readonly BddRuleset $ruleset, private readonly BddResultResolver $resultResolver)
    {
    }
    /**
    ...
    public function evaluate(array $inputParameters) : RulesetEndpoint
    {
        ...
        while (\true) {
            if ($ref >= self::RESULT_OFFSET) {
                return $this->resultResolver->resolve($ref - self::RESULT_OFFSET, $inputParameters);
            }
            ...
            $value = $library->callFunction($conditions[$condIndex], $inputParameters);
            $condResult = $value !== null && $value !== \false;
            // Complement edges invert the high/low selection without
            // duplicating nodes in the BDD.
            $ref = ($condResult xor $isComplement) ? $nodes[$base + 1] : $nodes[$base + 2];
        }
    }
}
