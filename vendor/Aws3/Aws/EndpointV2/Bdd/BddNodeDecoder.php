...
final class BddNodeDecoder
{
    ...
    {
        $bytes = \base64_decode($encoded, \true);
        if ($bytes === \false) {
            throw new UnresolvedEndpointException('Endpoint BDD nodes are not valid base64.');
        }
        $expectedBytes = $expectedNodeCount * self::BYTES_PER_NODE;
        if (\strlen($bytes) !== $expectedBytes) {
            throw new UnresolvedEndpointException(\sprintf('Endpoint BDD node payload is %d bytes but %d were expected' . ' for %d nodes.', \strlen($bytes), $expectedBytes, $expectedNodeCount));
        }
        ...
        foreach ($unsigned as $value) {
            $flat[] = $value > self::INT_32_MAX ? $value - self::INT_32_OFFSET : $value;
        }
        return $flat;
    }
}
