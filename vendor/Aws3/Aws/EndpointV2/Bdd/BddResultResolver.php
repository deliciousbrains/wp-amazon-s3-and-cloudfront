...
namespace DeliciousBrains\WP_Offload_Media\Aws3\Aws\EndpointV2\Bdd;
...
{
    public function __construct(private readonly BddRuleset $ruleset)
    {
    }
    /**
    ...
    public function resolve(int $resultIndex, array $inputParameters) : RulesetEndpoint
    {
        ...
        if ($result === null) {
            throw new UnresolvedEndpointException(\sprintf('Endpoint BDD referenced unknown result index %d.', $resultIndex));
        }
        if (isset($result['error'])) {
            $this->throwError($result, $inputParameters);
        }
        if (!isset($result['endpoint'])) {
            throw new UnresolvedEndpointException('Endpoint BDD result is missing an `endpoint` or `error` block.');
        }
    ...
    {
        $message = $this->ruleset->standardLibrary->resolveValue($result['error'], $inputParameters);
    ...
    private function buildEndpoint(array $endpoint, array $inputParameters) : RulesetEndpoint
    {
        ...
        $properties = isset($endpoint['properties']) ? $this->resolveProperties($endpoint['properties'], $inputParameters, $library) : null;
        $headers = isset($endpoint['headers']) ? $this->resolveHeaders($endpoint['headers'], $inputParameters, $library) : null;
        return new RulesetEndpoint($url, $properties, $headers);
    }
    private function resolveProperties($properties, array $inputParameters, RulesetStandardLibrary $library)
    {
        if (\is_array($properties)) {
            $resolved = [];
            foreach ($properties as $key => $value) {
                $resolved[$key] = $this->resolveProperties($value, $inputParameters, $library);
            }
            return $resolved;
        }
    ...
    private function resolveHeaders(array $headers, array $inputParameters, RulesetStandardLibrary $library) : array
    {
        $resolved = [];
