...
use DeliciousBrains\WP_Offload_Media\Aws3\Aws\EndpointV2\Ruleset\RulesetParameter;
...
{
    private const REQUIRED_FIELDS = ['conditions', 'results', 'nodes', 'root', 'nodeCount'];
    ...
    public function __construct(array $definition, array $partitions)
    {
        foreach (self::REQUIRED_FIELDS as $key) {
            if (!\array_key_exists($key, $definition)) {
                throw new UnresolvedEndpointException("Endpoint BDD definition is missing `{$key}`.");
            }
        }
        ...
        $this->nodes = BddNodeDecoder::decode((string) $definition['nodes'], $nodeCount);
