...
use DeliciousBrains\WP_Offload_Media\Aws3\Aws\EndpointV2\Bdd\BddEvaluator;
use DeliciousBrains\WP_Offload_Media\Aws3\Aws\EndpointV2\Bdd\BddResultResolver;
use DeliciousBrains\WP_Offload_Media\Aws3\Aws\EndpointV2\Bdd\BddRuleset;
...
class EndpointProviderV2
{
    ...
    public function __construct($ruleset, array $partitions)
    {
        if ($ruleset instanceof BddRuleset) {
            ...
            $this->bddEvaluator = new BddEvaluator($ruleset, new BddResultResolver($ruleset));
        } elseif ($ruleset instanceof Ruleset) {
            $this->ruleset = $ruleset;
        } elseif (\is_array($ruleset)) {
            $this->ruleset = new Ruleset($ruleset, $partitions);
        } else {
            throw new \InvalidArgumentException('EndpointProviderV2 expects an array, Ruleset, or BddRuleset' . ' but received ' . (\is_object($ruleset) ? \get_class($ruleset) : \gettype($ruleset)));
        }
    ...
    public function resolveEndpoint(array $inputParameters)
    {
        ...
        $endpoint = $this->bddEvaluator !== null ? $this->bddEvaluator->evaluate($inputParameters) : $this->ruleset->evaluate($inputParameters);
