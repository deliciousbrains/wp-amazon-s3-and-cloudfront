...
class RulesetStandardLibrary
{
    ...
    {
        if (!\is_string($arnString) || \strncmp($arnString, 'arn', 3) !== 0) {
            return null;
        }
        ...
        return ['partition' => $partition, 'service' => $service, 'region' => $region, 'accountId' => $accountId, 'resourceId' => \preg_split("/[:\\/]/", $resource)];
    }
    /**
    ...
    public function callFunction($funcCondition, &$inputParameters)
    {
        ...
        switch ($fn) {
            case 'aws.parseArn':
                $result = $this->parseArn($this->resolveValue($argv[0], $inputParameters));
                break;
            case 'getAttr':
                $result = $this->getAttr($this->resolveValue($argv[0], $inputParameters), $argv[1]);
                break;
            case 'stringEquals':
                $result = $this->stringEquals($this->resolveValue($argv[0], $inputParameters), $this->resolveValue($argv[1], $inputParameters));
                break;
            case 'booleanEquals':
                $result = $this->booleanEquals($this->resolveValue($argv[0], $inputParameters), $this->resolveValue($argv[1], $inputParameters));
                break;
            case 'isSet':
                $arg = $argv[0];
                $result = isset($arg['ref']) ? isset($inputParameters[$arg['ref']]) : $this->is_set($this->resolveValue($arg, $inputParameters));
                break;
            case 'not':
                $result = $this->not($this->resolveValue($argv[0], $inputParameters));
                break;
            case 'substring':
                $result = $this->substring($this->resolveValue($argv[0], $inputParameters), $this->resolveValue($argv[1], $inputParameters), $this->resolveValue($argv[2], $inputParameters), isset($argv[3]) ? $this->resolveValue($argv[3], $inputParameters) : \false);
                break;
            default:
                $funcArgs = [];
                foreach ($argv as $arg) {
                ...
                if (!\method_exists($this, $funcName)) {
                    throw new UnresolvedEndpointException("Unknown endpoint function `{$fn}`.");
                }
                $result = \call_user_func_array([$this, $funcName], $funcArgs);
        }
        if ($assign !== null) {
    ...
    public function resolveValue($value, $inputParameters)
    {
        ...
        } elseif (\is_string($value) && \str_contains($value, '{') && $this->isTemplate($value)) {
    ...
    {
        return \is_string($arg) && \str_contains($arg, '{') && \preg_match(self::TEMPLATE_SEARCH_RE, $arg) === 1;
    }
