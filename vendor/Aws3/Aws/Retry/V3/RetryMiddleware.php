...
namespace DeliciousBrains\WP_Offload_Media\Aws3\Aws\Retry\V3;
...
class RetryMiddleware
{
    ...
    private const THROTTLING_BASE_DELAY_MS = 1000;
    private const DEFAULT_BASE_DELAY_MS = 50;
    private const DEFAULT_MAX_BACKOFF_MS = 20000;
    private const RETRY_AFTER_HEADER = 'x-amz-retry-after';
    private static array $standardThrottlingErrors = ['Throttling' => \true, 'ThrottlingException' => \true, 'ThrottledException' => \true, 'RequestThrottledException' => \true, 'TooManyRequestsException' => \true, 'ProvisionedThroughputExceededException' => \true, 'TransactionInProgressException' => \true, 'RequestLimitExceeded' => \true, 'BandwidthLimitExceeded' => \true, 'LimitExceededException' => \true, 'RequestThrottled' => \true, 'SlowDown' => \true, 'PriorRequestNotComplete' => \true, 'EC2ThrottledException' => \true];
    private static array $standardTransientErrors = ['RequestTimeout' => \true, 'RequestTimeoutException' => \true];
    private static array $standardTransientStatusCodes = [500 => \true, 502 => \true, 503 => \true, 504 => \true];
    ...
    public static function wrap(ConfigurationInterface $config, array $options) : \Closure
    {
        ...
        if ($config->getMode() === 'adaptive' && !isset($options['rate_limiter'])) {
    ...
    {
        return function (int $attempts, CommandInterface $command, mixed $result) use($options) : bool {
            return self::isRetryable($result, $options);
        };
    }
    public function __construct(ConfigurationInterface $config, callable $handler, array $options = [])
    {
        ...
        $this->customDecider = isset($options['decider']) ? $options['decider'](...) : null;
        $this->delayer = isset($options['delayer']) ? $options['delayer'](...) : null;
    ...
    public function __invoke(CommandInterface $cmd, RequestInterface $req) : PromiseInterface
    {
        ...
        $callback = function ($value) use($handler, $cmd, $req, &$attempts, &$requestStats, &$monitoringEvents, &$callback, &$capacityUsed) {
            if ($this->mode === 'adaptive') {
            ...
            if (!$isError && $value instanceof ResultInterface) {
                $statusCode = isset($value['@metadata']['statusCode']) ? (int) $value['@metadata']['statusCode'] : null;
            ...
            $isRetryable = self::isRetryable($value, $this->options);
            ...
            $delayByMs = $this->computeRetryDelay($attemptIndex, $isThrottling, $value);
            ...
            $req = $this->addRetryHeader($req, $attempts - 1, $maxAttempts);
            if ($this->mode === 'adaptive') {
    ...
    {
        $baseMs = $isThrottling ? self::THROTTLING_BASE_DELAY_MS : $this->baseDelayMs;
        ...
        $delayMs = $jitter * \min($baseMs * \pow(2, $attemptIndex), $this->maxBackoffMs);
        $retryAfterHeader = null;
        if ($value instanceof AwsException) {
            $response = $value->getResponse();
            $retryAfterHeader = $response?->getHeaderLine(self::RETRY_AFTER_HEADER) ?? null;
        } elseif ($value instanceof ResultInterface) {
            $retryAfterHeader = $value['@metadata']['headers'][self::RETRY_AFTER_HEADER] ?? null;
        }
    ...
    private static function isRetryable(mixed $result, array $options = []) : bool
    {
    ...
    {
        return $cmd['@retries'] !== null ? $cmd['@retries'] + 1 : $this->maxAttempts;
    }
}
