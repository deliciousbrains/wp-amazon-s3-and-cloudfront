...
use DeliciousBrains\WP_Offload_Media\Aws3\Aws\S3\Exception\MultipartCopyAnnotationException;
...
class MultipartCopy extends AbstractUploadManager
{
    ...
    private const VALID_METADATA_DIRECTIVES = ['COPY' => \true, 'REPLACE' => \true];
    ...
    private const VALID_TAGS_DIRECTIVES = [self::TAGS_DIRECTIVE_UNSPECIFIED => \true, self::TAGS_DIRECTIVE_COPY => \true, self::TAGS_DIRECTIVE_REPLACE => \true];
    ...
    private const VALID_ANNOTATIONS_DIRECTIVES = [self::ANNOTATIONS_DIRECTIVE_UNSPECIFIED => \true, self::ANNOTATIONS_DIRECTIVE_COPY => \true, self::ANNOTATIONS_DIRECTIVE_EXCLUDE => \true];
    ...
    private static array $copyMetadataFields = ['CacheControl', 'ContentDisposition', 'ContentEncoding', 'ContentLanguage', 'ContentType', 'Expires', 'Metadata'];
    /** @var string|array */
    private $source;
    ...
    public function __construct(S3ClientInterface $client, $source, array $config = [])
    {
        ...
        parent::__construct($client, $config + ['source_metadata' => null]);
        ...
        return $this->promise = Coroutine::of(function () {
            try {
                ...
                if (!$this->state->isInitiated()) {
                    ...
                    $this->state->setUploadId($this->info['id']['upload_id'], $init[$this->info['id']['upload_id']]);
                    $this->state->setStatus(UploadState::INITIATED);
                }
                // Phase 2: parts.
                $resultHandler = $this->getResultHandler($errors);
                $pool = new CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
                (yield $pool->promise());
                if ($errors) {
    ...
    protected function getInitiateParams()
    {
        ...
        if (!isset(self::VALID_METADATA_DIRECTIVES[$directive])) {
            throw new \InvalidArgumentException("Invalid metadata_directive value '{$directive}'." . " Must be 'COPY' or 'REPLACE'.");
        }
    ...
    {
        if (!$this->sourceMetadata instanceof ResultInterface && $this->config['source_metadata'] instanceof ResultInterface) {
        ...
        return $this->client->headObjectAsync($this->buildHeadParams())->then(function (ResultInterface $r) : ResultInterface {
    ...
    {
        if (\is_array($this->source)) {
            $p = ['Bucket' => $this->source['source_bucket'], 'Key' => $this->source['source_key']];
        } else {
            [$bucket, $key] = \explode('/', \ltrim($this->source, '/'), 2);
            if (\str_contains($key, '?')) {
                [$key] = \explode('?', $key, 2);
            }
            $p = ['Bucket' => $bucket, 'Key' => \rawurldecode($key)];
        }
    ...
    {
        return $this->client->getObjectTaggingAsync($this->buildSourceObjectParams())->then(function (ResultInterface $r) : ResultInterface {
    ...
    private function fetchSourceAnnotations() : PromiseInterface
    {
        return Coroutine::of(function () {
            ...
            $names = [];
            (yield $this->client->getPaginator('ListObjectAnnotations', $listParams)->each(function (ResultInterface $page) use(&$names) {
            ...
            $commands = \array_map(function ($name) use($getParams) {
                return $this->client->getCommand('GetObjectAnnotation', $getParams + ['AnnotationName' => $name]);
            }, $names);
            $pool = new CommandPool($this->client, $commands, ['concurrency' => $this->config['concurrency'], 'fulfilled' => function (ResultInterface $result, $iterKey) use($names) {
                $name = $names[$iterKey];
            ...
            }, 'rejected' => function ($reason, $iterKey, PromiseInterface $aggregatePromise) {
     ...
     * @return \Generator
     */
    private function writeDestinationTags(string $tagsDirective, ?string $destVersionId) : \Generator
    {
        ...
        if ($tagsDirective === self::TAGS_DIRECTIVE_REPLACE) {
            ...
            $tagging = \is_array($callerTagging) ? $callerTagging : ['TagSet' => $this->parseTaggingQueryString((string) $callerTagging)];
            $params = ['Bucket' => $destBucket, 'Key' => $destKey, 'Tagging' => $tagging];
        ...
        $params = ['Bucket' => $destBucket, 'Key' => $destKey, 'Tagging' => ['TagSet' => $this->sourceTags]];
        if ($destVersionId !== null) {
            $params['VersionId'] = $destVersionId;
        }
        (yield $this->client->putObjectTaggingAsync($params));
     ...
     */
    private function writeDestinationAnnotations(?string $destETag, ?string $destVersionId) : \Generator
    {
        ...
        $failed = [];
        [$destBucket, $destKey] = $this->resolveDestinationBucketAndKey();
        $putAnnotationsCalls = function () use($destBucket, $destKey, $destETag, $destVersionId, &$succeeded, &$failed) {
            foreach ($this->annotationBodies as $name => $body) {
                $params = ['Bucket' => $destBucket, 'Key' => $destKey, 'AnnotationName' => $name, 'AnnotationPayload' => $body];
                if ($destVersionId !== null) {
                    $params['VersionId'] = $destVersionId;
                }
                if ($destETag !== null) {
                ...
                (yield $this->putAnnotationWithRetries($params)->then(function () use($name, &$succeeded) {
                    $succeeded[] = $name;
        ...
        if ($failed) {
            throw new MultipartCopyAnnotationException($this->state, $failed, $succeeded);
        }
    }
    /**
    ...
    private function resolveTagsDirective() : string
    {
        ...
        if (!isset(self::VALID_TAGS_DIRECTIVES[$value])) {
            throw new \InvalidArgumentException("Invalid tags_directive value '{$value}'. Must be one of: " . \implode(', ', \array_keys(self::VALID_TAGS_DIRECTIVES)) . '.');
        }
        return $value;
    }
    /**
    ...
    private function resolveAnnotationsDirective() : string
    {
        ...
        if (!isset(self::VALID_ANNOTATIONS_DIRECTIVES[$value])) {
            throw new \InvalidArgumentException("Invalid annotations_directive value '{$value}'. Must be one of: " . \implode(', ', \array_keys(self::VALID_ANNOTATIONS_DIRECTIVES)) . '.');
        }
        return $value;
    }
    /**
