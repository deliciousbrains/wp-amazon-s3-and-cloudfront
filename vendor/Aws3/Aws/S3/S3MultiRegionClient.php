    ...
    private function determineRegionMiddleware()
    {
        ...
        return static function (callable $handler) use($clientRef) {
            return static function (CommandInterface $command) use($handler, $clientRef) {
                ...
                if (empty($command['@region']) && ($region = $client->cache->get($cacheKey))) {
                    $command['@region'] = $region;
                }
                return Promise\Coroutine::of(static function () use($handler, $command, $cacheKey, $clientRef) {
                    try {
                        ...
                        if (isset($result['@metadata']['headers']['x-amz-bucket-region'])) {
                            ...
                            $region = (yield $client->determineBucketRegionAsync($command['Bucket']));
                        }
                        $command['@region'] = $region;
                        (yield $handler($command));
                        ...
                        if ($e->getAwsErrorCode() === 'AuthorizationHeaderMalformed') {
                            $client = $clientRef->get();
                            $region = $client->determineBucketRegionFromExceptionBody($e->getResponse());
                            if (!empty($region)) {
                                $client->cache->set($cacheKey, $region);
                                $command['@region'] = $region;
                                (yield $handler($command));
