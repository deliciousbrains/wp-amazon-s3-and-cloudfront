...
use DeliciousBrains\WP_Offload_Media\Aws3\GuzzleHttp\Auth\DigestAuth;
use DeliciousBrains\WP_Offload_Media\Aws3\GuzzleHttp\Auth\DigestChallenge;
...
final class AuthMiddleware
{
     ...
     * @var list<string>
     */
    private const DIGEST_PROBE_PAYLOAD_HEADERS = ['Transfer-Encoding', 'Expect', 'Trailer', 'Content-Range', 'Content-Encoding', 'Content-MD5', 'Digest', 'Content-Digest', 'Repr-Digest'];
    /**
     ...
     */
    public function __construct(callable $nextHandler, ?callable $cnonceGenerator = null, bool $reuseChallenges = \true)
    {
     ...
     */
    private static function normalizeAuth(#[\SensitiveParameter] array $auth) : ?array
    {
        $type = 'basic';
        ...
        if (!\in_array($type, ['basic', 'digest'], \true)) {
            return null;
        }
     ...
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private function sendBasic(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options, string $username, #[\SensitiveParameter] string $password) : PromiseInterface
    {
        ...
        unset($options['auth']);
        return ($this->nextHandler)($request->withHeader('Authorization', 'Basic ' . \base64_encode($username . ':' . $password)), $options);
    }
    /**
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private function sendDigest(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options, string $username, #[\SensitiveParameter] string $password) : PromiseInterface
    {
        $preemptive = $this->reuseChallenges ? $this->preemptiveDigestRequest($request, $username, $password) : null;
        if ($preemptive !== null) {
            $preemptiveOptions = self::withTemporarySink($options);
            unset($preemptiveOptions['auth']);
            return ($this->nextHandler)($preemptive, $preemptiveOptions)->then(function (#[\SensitiveParameter] ResponseInterface $response) use($request, $options, $preemptiveOptions, $username, $password) {
            ...
            }, function (#[\SensitiveParameter] $reason) use($request, $preemptiveOptions) {
                return $this->handleDigestRejection($request, $preemptiveOptions, $reason, \true);
            });
        }
        ...
        return ($this->nextHandler)($probeRequest, $probeOptions)->then(function (#[\SensitiveParameter] ResponseInterface $response) use($request, $options, $probeOptions, $username, $password, $bodyWithheld) {
            return $this->handleDigestResponse($request, $options, $probeOptions, $response, $username, $password, $bodyWithheld);
        }, function (#[\SensitiveParameter] $reason) use($request, $probeOptions) {
            return $this->handleDigestRejection($request, $probeOptions, $reason, \false);
        });
    }
    private function preemptiveDigestRequest(#[\SensitiveParameter] RequestInterface $request, string $username, #[\SensitiveParameter] string $password) : ?RequestInterface
    {
        try {
        ...
        if ($entry === null || $entry['credentials'] !== $this->digestCredentialKey($username, $password) || !self::challengeCoversRequest($entry['challenge'], $request)) {
            return null;
        }
        $authorization = DigestAuth::authorizationHeader($request, $entry['challenge'], $username, $password, ($this->cnonceGenerator)(), \sprintf('%08x', $entry['nextNc']));
     ...
     */
    private static function probeRequest(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options) : array
    {
        ...
        $probe = $probe->withBody($streamFactory->createStream(''))->withHeader('Content-Length', '0');
     ...
     */
    private function handleDigestResponse(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options, #[\SensitiveParameter] array $probeOptions, #[\SensitiveParameter] ResponseInterface $response, string $username, #[\SensitiveParameter] string $password, bool $bodyWithheld)
    {
        $status = $response->getStatusCode();
        $previousEntry = null;
        ...
        if ($status !== 401) {
            ...
            if ($bodyWithheld && $status !== 407 && !self::isRedirectLikeResponse($response)) {
                throw new ResponseException('Digest authentication failed because the server did not issue a challenge; the request was probed without its body', $request, $response);
            }
            return $response;
        }
        ...
        try {
            ...
            $response = self::restoreOriginalSink($request, $response, $probeOptions);
            throw new ResponseException('Digest authentication failed because the request body could not be rewound', $request, $response, $e);
        }
        $nonceCounts = self::digestNonceCountsFromOptions($options);
        if ($previousEntry !== null && $previousEntry['credentials'] === $this->digestCredentialKey($username, $password)) {
        ...
        $authorization = DigestAuth::authorizationHeader($request, $challenge, $username, $password, ($this->cnonceGenerator)(), \sprintf('%08x', $nc));
        if ($authorization === null) {
            return self::restoreOriginalSink($request, $response, $probeOptions);
        }
        ...
        return ($this->nextHandler)($retryRequest, $downstreamOptions)->then(function (#[\SensitiveParameter] ResponseInterface $retryResponse) use($retryRequest, $retryOptions, $downstreamOptions, $username, $password, $challenge, $nc) {
        ...
        }, function (#[\SensitiveParameter] $reason) use($retryRequest, $downstreamOptions) {
    ...
    private function storeDigestChallenge(#[\SensitiveParameter] RequestInterface $request, DigestChallenge $challenge, string $username, #[\SensitiveParameter] string $password, int $nextNc) : void
    {
        $key = self::digestCacheKey($request);
        ...
        $this->digestChallenges[$key] = ['challenge' => $challenge, 'credentials' => $this->digestCredentialKey($username, $password), 'nextNc' => $nextNc];
    }
    ...
    {
        return \implode("\x00", [$challenge->realm, $challenge->nonce, $challenge->qop ?? '', $challenge->algorithm['header']]);
    }
    /**
     ...
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private function handleDigestRejection(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options, #[\SensitiveParameter] $reason, bool $preemptive) : PromiseInterface
    {
        if ($this->reuseChallenges && $reason instanceof ResponseException) {
            $response = $reason->getResponse();
            if ($response->getStatusCode() === 401 || $response->hasHeader('Authentication-Info') || $preemptive && $response->getStatusCode() >= 400) {
                $this->clearDigestChallenge($request);
            }
        }
    ...
    private function digestCredentialKey(string $username, #[\SensitiveParameter] string $password) : string
    {
        if ($this->credentialHashSecret === null) {
            $this->credentialHashSecret = \random_bytes(32);
        }
        return \hash_hmac('sha256', $username . "\x00" . $password, $this->credentialHashSecret);
    }
    ...
    {
        return $response->hasHeader('Location') && \in_array($response->getStatusCode(), [301, 302, 303, 307, 308], \true);
    }
    private static function rewindBodyForRetry(#[\SensitiveParameter] RequestInterface $request, bool $bodyWithheld) : void
    {
    ...
    private static function withTemporarySink(#[\SensitiveParameter] array $options) : array
    {
        ...
        $options[RequestOptions::SINK] = $streamFactory->createStreamFromResource(Psr7\Utils::tryFopen('php://temp', 'w+'));
        return $options;
    }
    private static function restoreOriginalSink(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] ResponseInterface $response, #[\SensitiveParameter] array $options) : ResponseInterface
    {
     ...
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private static function restoreOriginalSinkOnRejection(#[\SensitiveParameter] array $options, #[\SensitiveParameter] $reason) : PromiseInterface
    {
    ...
    private static function streamForOriginalSink($sink) : StreamInterface
    {
        ...
        if (!$sink instanceof StreamInterface) {
            throw new InvalidArgumentException(\sprintf('sink must be a resource, string, or %s', StreamInterface::class));
        }
