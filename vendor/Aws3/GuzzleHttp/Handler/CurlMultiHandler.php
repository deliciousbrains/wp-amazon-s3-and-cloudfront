...
use Closure;
use DeliciousBrains\WP_Offload_Media\Aws3\GuzzleHttp\Exception\HandlerClosedException;
...
final class CurlMultiHandler
{
    ...
    public function __construct(array $options = [])
    {
        ...
        if ($connectionCapOption !== null) {
            $persistentShareState = $transportSharing instanceof CurlShareHandleState && \in_array($sharingMode, [TransportSharing::PERSISTENT_PREFER, TransportSharing::PERSISTENT_REQUIRE], \true);
     ...
     * nothing waits.
     */
    private function rejectDisabledMultiplexConflict(#[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array $options) : void
    {
        if (!$this->multiplexDisabled) {
            return;
        }
     ...
     * fresh connection.
     */
    private function applyMultiplexNone(#[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array $options) : void
    {
        ...
        if (\defined('CURLOPT_HTTPAUTH') && isset($options['curl']) && \is_array($options['curl']) && \array_key_exists((int) \constant('CURLOPT_HTTPAUTH'), $options['curl'])) {
     ...
     */
    private function applyProxyTunnelOwnership(#[\SensitiveParameter] EasyHandle $easy) : void
    {
        ...
        if ($this->handles === [] && 0 === $this->multiExecDepth && 0 === $this->messageProcessingDepth && $this->deferredCancels === [] && !$this->deferredClose) {
    ...
    private function addHandleToMulti(int $id, #[\SensitiveParameter] EasyHandle $easy) : void
    {
    ...
    private function isolateFromForeignActiveProxyTunnel(#[\SensitiveParameter] EasyHandle $easy) : void
    {
        $signature = $easy->proxyTunnelSignature;
    ...
    private function isolateProxyTunnelTransfer(#[\SensitiveParameter] EasyHandle $easy) : void
    {
    ...
    private function executeUntil(int $id, object $waitToken) : string
    {
        $this->assertOpen();
        $queue = P\Utils::queue();
        while (!$this->closed && !$this->closing && $this->hasRequest($id, $waitToken)) {
    ...
    private function deferClose(bool $explicit, ?\Throwable &$failure) : void
    {
        ...
        foreach ($entries as $id => $entry) {
            $this->deferredCancels[$id] = ['easy' => $entry['easy'], 'attached' => !empty($entry['attached'])];
    ...
    private function failNestedWait(int $id, object $token) : string
    {
        ...
        $failure = $response !== null ? new ResponseException($message, $entry['easy']->request, $response) : new RequestException($message, $entry['easy']->request);
    ...
    private function disposeEasyHandle(#[\SensitiveParameter] EasyHandle $easy) : void
    {
    ...
    private function addRequest(#[\SensitiveParameter] array $entry) : void
    {
        $easy = $entry['easy'];
        $id = (int) $easy->handle;
        ...
        try {
            foreach ($this->options as $option => $value) {
                ...
                try {
                    ...
                    throw new InvalidArgumentException(\sprintf('Unable to apply the cURL multi option %s; it was rejected by the runtime libcurl.', self::formatCurlMultiOption($option)), 0, $e);
                }
                if (\true !== $applied) {
