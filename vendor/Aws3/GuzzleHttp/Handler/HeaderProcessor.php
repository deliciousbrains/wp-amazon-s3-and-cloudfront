...
final class HeaderProcessor
{
    ...
    public static function contentLengthToInt(?string $length) : ?int
    {
        ...
        if (\strlen($length) > \strlen($max) || \strlen($length) === \strlen($max) && \strcmp($length, $max) > 0) {
            return null;
        }
        return (int) $length;
    }
     ...
     */
    public static function validateResponseFraming(string $method, int $status, array $headers) : ?string
    {
    ...
    {
        return $method !== 'HEAD' && !($method === 'CONNECT' && $status >= 200 && $status < 300) && $status >= 200 && $status !== 204 && $status !== 304;
    }
    /**
