...
class Pool implements PromisorInterface
{
     ...
     */
    public function __construct(ClientInterface $client, iterable $requests, #[\SensitiveParameter] array $config = [])
    {
    ...
    private static function cmpCallback(array &$options, string $name, array &$results) : void
    {
        if (!isset($options[$name])) {
            $options[$name] = static function (#[\SensitiveParameter] $v, $k) use(&$results) : void {
                $results[$k] = $v;
            };
        } else {
            $currentFn = $options[$name];
            $options[$name] = static function (#[\SensitiveParameter] $v, $k) use(&$results, $currentFn) : void {
                $currentFn($v, $k);
                $results[$k] = $v;
            };
        }
    ...
    {
        if (\is_array($options['allow_redirects'] ?? null) && \is_callable($options['allow_redirects']['on_redirect'] ?? null)) {
            $onRedirect = $options['allow_redirects']['on_redirect'];
            $options['allow_redirects']['on_redirect'] = static function (#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] ResponseInterface $response, #[\SensitiveParameter] UriInterface $uri) use($onRedirect, $key) : void {
                $onRedirect($request, $response, $uri, $key);
            };
        }
        if (\is_callable($options['on_headers'] ?? null)) {
            $onHeaders = $options['on_headers'];
            $options['on_headers'] = static function (#[\SensitiveParameter] ResponseInterface $response, #[\SensitiveParameter] RequestInterface $request) use($onHeaders, $key) : void {
        ...
        if (\is_callable($options['on_trailers'] ?? null)) {
            ...
            $options['on_trailers'] = static function (array $trailers, #[\SensitiveParameter] ResponseInterface $response, #[\SensitiveParameter] RequestInterface $request) use($onTrailers, $key) : void {
