    ...
    public function __invoke(\DeliciousBrains\WP_Offload_S3\Aws3\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \DeliciousBrains\WP_Offload_S3\Aws3\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\DeliciousBrains\WP_Offload_S3\Aws3\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    private function addExpectHeader(\DeliciousBrains\WP_Offload_S3\Aws3\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
