...
class RedirectMiddleware
{
     ...
     * @var array
     */
    public const DEFAULT_SETTINGS = ['max' => 5, 'protocols' => ['http', 'https'], 'strict' => \false, 'referer' => \false, 'track_redirects' => \false];
    /**
    ...
    public function __invoke(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options) : PromiseInterface
    {
        ...
        return $fn($request, $options)->then(function (#[\SensitiveParameter] ResponseInterface $response) use($request, $options) {
     ...
     */
    public function checkRedirect(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options, #[\SensitiveParameter] ResponseInterface $response)
    {
        if (!self::isRedirectStatusCode($response->getStatusCode()) || !$response->hasHeader('Location')) {
            return $response;
        }
     ...
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private function withTracking(PromiseInterface $promise, #[\SensitiveParameter] string $uri, int $statusCode) : PromiseInterface
    {
        return $promise->then(static function (#[\SensitiveParameter] ResponseInterface $response) use($uri, $statusCode) : ResponseInterface {
     ...
     */
    private function guardMax(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] ResponseInterface $response, #[\SensitiveParameter] array &$options) : void
    {
    ...
    public function modifyRequest(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options, #[\SensitiveParameter] ResponseInterface $response) : RequestInterface
    {
        ...
        if (!isset($modify['body'])) {
            try {
                ...
                throw new ResponseException('Redirect failed because the request body could not be rewound', $request, $response, $e);
            }
        }
    ...
    {
        return \in_array($statusCode, [301, 302, 303, 307, 308], \true);
    }
    /**
     * @return array{
     *     method?: string,
     ...
     * }
     */
    private static function getRedirectRequestModifiers(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options, #[\SensitiveParameter] ResponseInterface $response) : array
    {
        ...
        return ['method' => \in_array($requestMethod, ['GET', 'HEAD', 'OPTIONS'], \true) ? $requestMethod : 'GET', 'body' => $streamFactory->createStream(''), 'remove_headers' => ['Content-Length', 'Transfer-Encoding']];
    }
    /**
     * Set the appropriate URL on the request based on the location header.
     */
    private static function redirectUri(UriFactoryInterface $uriFactory, #[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] ResponseInterface $response, array $protocols) : UriInterface
    {
        $location = $response->getHeaderLine('Location');
        try {
            $locationUri = $uriFactory->createUri($location);
            $resolvedUri = Psr7\UriResolver::resolve($request->getUri(), $locationUri);
            if (!$uriFactory instanceof HttpFactory && $locationUri->getScheme() === '' && $locationUri->getAuthority() === '') {
        ...
        if (!\in_array($resolvedUri->getScheme(), $protocols, \true)) {
