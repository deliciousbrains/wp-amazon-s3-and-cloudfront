...
class HashValidatingStream implements StreamInterface
{
    ...
    {
        if ($this->crc32cEnabled) {
            $crc32cHash = \hash_final($this->crc32cContext, \true);
            $calculatedCrc32c = \base64_encode($crc32cHash);
            $this->crc32cEnabled = \false;
            // Prevent double validation
            if ($calculatedCrc32c !== $this->expectedCrc32c) {
                throw new UnexpectedValueException(\sprintf('CRC32C checksum mismatch. Expected: %s, Calculated: %s', $this->expectedCrc32c, $calculatedCrc32c));
            }
        }
        if ($this->md5Enabled) {
            $md5Hash = \hash_final($this->md5Context, \true);
            $calculatedMd5 = \base64_encode($md5Hash);
            $this->md5Enabled = \false;
            // Prevent double validation
            if ($calculatedMd5 !== $this->expectedMd5) {
                throw new UnexpectedValueException(\sprintf('MD5 checksum mismatch. Expected: %s, Calculated: %s', $this->expectedMd5, $calculatedMd5));
            }
        }
    }
}
