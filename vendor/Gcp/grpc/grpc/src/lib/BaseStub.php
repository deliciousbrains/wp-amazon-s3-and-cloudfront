 ...
 *
 */
namespace DeliciousBrains\WP_Offload_Media\Gcp\Grpc;
...
class BaseStub
{
    ...
    {
        if (!\method_exists('DeliciousBrains\\WP_Offload_Media\\Gcp\\Grpc\\ChannelCredentials', 'isDefaultRootsPemSet') || !ChannelCredentials::isDefaultRootsPemSet()) {
        ...
        if ($channel) {
            if (!\is_a($channel, 'Grpc\\Channel') && !\is_a($channel, 'DeliciousBrains\\WP_Offload_Media\\Gcp\\Grpc\\Internal\\InterceptorChannel')) {
    ...
    private static function updateOpts($opts)
    {
        ...
        if (\defined('DeliciousBrains\\WP_Offload_Media\\Gcp\\Grpc\\VERSION')) {
            $version_str = \DeliciousBrains\WP_Offload_Media\Gcp\Grpc\VERSION;
        } else {
    ...
    {
        if (\is_a($channel, 'DeliciousBrains\\WP_Offload_Media\\Gcp\\Grpc\\Internal\\InterceptorChannel')) {
            return function ($method, $argument, $deserialize, array $metadata = [], array $options = []) use($channel) {
