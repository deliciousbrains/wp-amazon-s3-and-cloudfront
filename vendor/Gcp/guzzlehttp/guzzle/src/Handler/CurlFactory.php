...
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Exception\ConnectTimeoutException;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Exception\InvalidArgumentException;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Exception\NetworkException;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Exception\NetworkTimeoutException;
...
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Exception\ResponseTimeoutException;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Exception\ResponseTransferException;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Multiplexing;
...
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\ProxyOptions;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7\Exception\TimeoutException;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7\FnStream;
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7\HttpFactory;
...
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\TransportSharing;
...
final class CurlFactory implements CurlFactoryInterface
{
    ...
    private const CURL_CONNECTION_ERRORS = [
        ...
        51 => \true,
        // CURLE_PEER_FAILED_VERIFICATION before libcurl 7.62.0
        60 => \true,
        // CURLE_SSL_CACERT / modern CURLE_PEER_FAILED_VERIFICATION
        83 => \true,
    ...
    private const CURL_CONNECT_TIMEOUT_ERRORS = ['Connection timed out', 'Connection timeout', 'Connection time-out', 'Resolving timed out', 'name lookup timed out', 'Proxy CONNECT aborted due to timeout', 'SSL connection timeout'];
    /**
    ...
    public function __construct(int $maxHandles, string $shareMode = TransportSharing::NONE, $shareHandle = null)
    {
        ...
        if ($shareHandle instanceof CurlShareHandleState) {
            ...
            $this->opaqueShareConnectionCache = $shareHandle->mode === TransportSharing::PERSISTENT_PREFER || $shareHandle->mode === TransportSharing::PERSISTENT_REQUIRE;
    ...
    private static function isCurlShareHandle($value) : bool
    {
        ...
        return \class_exists('DeliciousBrains\\WP_Offload_Media\\Gcp\\CurlSharePersistentHandle') && $value instanceof \DeliciousBrains\WP_Offload_Media\Gcp\CurlSharePersistentHandle;
    }
     ...
     */
    private function applyCurlOptions($handle, #[\SensitiveParameter] array $conf) : void
    {
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function normalizeStringableProxyCredentialOptions(#[\SensitiveParameter] array &$conf) : void
    {
    ...
    private function rejectRequestLevelShareConflict(#[\SensitiveParameter] array $options) : void
    {
        if ($this->shareHandle === null) {
            return;
        }
    ...
    private function rejectPersistentRequireConnectionReuseConflicts(#[\SensitiveParameter] array $options) : void
    {
        if ($this->shareMode !== TransportSharing::PERSISTENT_REQUIRE || !isset($options['curl']) || !\is_array($options['curl'])) {
            return;
        }
    ...
    private static function normalizeMultiplex(#[\SensitiveParameter] array $options) : string
    {
    ...
    private static function assertRequiredMultiplexSupported(#[\SensitiveParameter] EasyHandle $easy) : void
    {
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function assertRequiredMultiplexAuthSupported(#[\SensitiveParameter] array $conf) : void
    {
    ...
    private static function assertSelectedProxySupported(#[\SensitiveParameter] ?string $selectedProxy, #[\SensitiveParameter] RequestInterface $request) : void
    {
        if ($selectedProxy === null || $selectedProxy === '') {
            return;
        }
        $scheme = ProxyOptions::proxyScheme($selectedProxy);
        if (!\in_array($scheme, ['http', 'https', 'socks4', 'socks4a', 'socks5', 'socks5h'], \true)) {
    ...
    private static function rejectConflictingCurlOptions(#[\SensitiveParameter] array $options) : void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                throw new InvalidArgumentException(\sprintf('Passing %s in the "curl" request option is not supported because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
            }
            throw new InvalidArgumentException(\sprintf('Passing %s in the "curl" request option is not supported because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function rejectUnsupportedCurlOptions(#[\SensitiveParameter] array $options) : void
    {
        if (!isset($options['curl']) || !\is_array($options['curl']) || $options['curl'] === []) {
            return;
        }
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            throw new InvalidArgumentException(\sprintf('Passing %s in the "curl" request option is not supported because it is outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function rejectUnsupportedRequestOptions(#[\SensitiveParameter] array $options) : void
    {
    ...
    private static function assertOnStatsCallable(#[\SensitiveParameter] array $options) : void
    {
        if (isset($options['on_stats']) && !\is_callable($options['on_stats'])) {
            throw new InvalidArgumentException('on_stats must be callable');
        }
    }
    private static function assertOnTrailersCallable(#[\SensitiveParameter] array $options) : void
    {
    ...
    private static function validateRequestUriScheme(#[\SensitiveParameter] RequestInterface $request) : void
    {
    ...
    private function clearEasyHandleCallbacks($handle) : void
    {
        ...
        if (\defined('DeliciousBrains\\WP_Offload_Media\\Gcp\\CURLOPT_PREREQFUNCTION')) {
     ...
     */
    public static function finish(callable $handler, #[\SensitiveParameter] EasyHandle $easy, CurlFactoryInterface $factory) : PromiseInterface
    {
        ...
        try {
            ...
            $reason = new ResponseException($e->getMessage() !== '' ? $e->getMessage() : 'Failed to rewind the response body', $easy->request, $response, $e);
            if ($onStats !== null && $stats !== null) {
                ...
                $onStats(new TransferStats($easy->request, $response, $stats->getTransferTime(), $reason, $stats->getHandlerStats()));
            }
        ...
        if (isset($easy->options['on_trailers'])) {
            ...
            try {
                ...
                $reason = new ResponseException('An error was encountered during the on_trailers event', $easy->request, $response, $e);
                if ($onStats !== null && $stats !== null) {
    ...
    private static function createStats(#[\SensitiveParameter] EasyHandle $easy) : TransferStats
    {
        $curlStats = \curl_getinfo($easy->handle);
        $curlStats['appconnect_time'] = \curl_getinfo($easy->handle, \CURLINFO_APPCONNECT_TIME);
        if ($easy->createResponseException) {
            $curlStats = ['total_time' => $curlStats['total_time'], 'appconnect_time' => $curlStats['appconnect_time']];
        }
        $handlerErrorData = $easy->responseHeaderException ?? $easy->responseBodySizeException ?? $easy->errno;
        return new TransferStats($easy->request, $easy->response, $curlStats['total_time'], $handlerErrorData, $curlStats);
    }
    /**
     * @param callable(RequestInterface, array<array-key, mixed>): PromiseInterface<ResponseInterface, mixed> $handler
     ...
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private static function finishError(callable $handler, #[\SensitiveParameter] EasyHandle $easy, CurlFactoryInterface $factory, ?TransferStats $stats, ?callable $onStats) : PromiseInterface
    {
        ...
        if (self::isChallengeRewindFailure($easy)) {
            $ctx['error'] = 'The server issued an authentication challenge ' . 'after the request body had already been sent, and the body ' . 'could not be rewound to resend it. The request was not ' . 'retried because a retry replays the same challenge. See ' . 'https://bugs.php.net/bug.php?id=47204 for more information.';
        }
        return self::createRejection($easy, $ctx);
    }
    private static function shouldFinishWithError(EasyHandle $easy) : bool
    {
        return !$easy->response || $easy->errno !== 0 || self::hasLocalFailure($easy);
    }
    private static function hasLocalFailure(EasyHandle $easy) : bool
    {
        return $easy->bodyReadTimeoutException !== null || $easy->bodyReadException !== null || $easy->responseHeaderException !== null || $easy->sinkWriteTimeoutException !== null || $easy->sinkWriteException !== null || $easy->sinkWriteIncomplete || $easy->responseBodySizeException !== null;
    }
    ...
    {
        return $easy->errno === self::CURLE_SEND_FAIL_REWIND && $easy->response !== null && !self::hasLocalFailure($easy);
    }
    private static function createErrorContext(EasyHandle $easy) : array
    {
        return ['errno' => $easy->errno, 'error' => \curl_error($easy->handle)];
    }
    /**
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private static function createRejection(#[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array $ctx, #[\SensitiveParameter] ?\Throwable $previous = null) : PromiseInterface
    {
        if ($easy->createResponseException) {
            /** @var PromiseInterface<ResponseInterface, mixed> */
            return P\Create::rejectionFor(new RequestException('An error was encountered while creating the response', $easy->request, 0, $easy->createResponseException));
        }
        ...
        if ($easy->onHeadersException) {
            return self::createRequestOrResponseRejection($easy, 'An error was encountered during the on_headers event', $easy->onHeadersException);
        }
        if ($easy->progressException) {
            return self::createRequestOrResponseRejection($easy, 'An error was encountered during the progress event', $easy->progressException);
        }
        if ($easy->bodyReadTimeoutException) {
            // Reading the request body stalled, which is a caller-stream failure.
            return self::createRequestOrResponseRejection($easy, 'Timed out while reading the request body', $easy->bodyReadTimeoutException);
        }
        if ($easy->bodyReadException) {
            $message = $easy->bodyReadException->getMessage() !== '' ? $easy->bodyReadException->getMessage() : 'Failed to read the request body';
        ...
        if ($easy->sinkWriteTimeoutException) {
            ...
            return self::createRequestOrResponseRejection($easy, 'Timed out while writing the response body', $easy->sinkWriteTimeoutException);
        }
        if ($easy->sinkWriteException) {
            $message = $easy->sinkWriteException->getMessage() !== '' ? $easy->sinkWriteException->getMessage() : 'Failed to write the response body';
        ...
        if ($easy->responseBodySizeException) {
            return self::createRequestOrResponseRejection($easy, $easy->responseBodySizeException->getMessage(), $easy->responseBodySizeException);
        }
        if ($easy->progressAborted && $easy->errno === \CURLE_ABORTED_BY_CALLBACK) {
            return self::createRequestOrResponseRejection($easy, 'The transfer was aborted by the progress callback');
        }
        ...
        if ($easy->errno === \CURLE_OPERATION_TIMEOUTED) {
            ...
            $error = self::isResponseTransferError($easy->errno) ? new ResponseTransferException($message, $easy->request, $easy->response, $previous) : new ResponseException($message, $easy->request, $easy->response, $previous);
     ...
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private static function createRequestOrResponseRejection(#[\SensitiveParameter] EasyHandle $easy, string $message, #[\SensitiveParameter] ?\Throwable $previous = null) : PromiseInterface
    {
        if ($easy->response !== null) {
            /** @var PromiseInterface<ResponseInterface, mixed> */
            return P\Create::rejectionFor(new ResponseException($message, $easy->request, $easy->response, $previous));
        }
        /** @var PromiseInterface<ResponseInterface, mixed> */
        return P\Create::rejectionFor(new RequestException($message, $easy->request, 0, $previous));
    }
    ...
    {
        return self::isConnectionError($errno) || self::isNetworkError($errno) || isset(self::CURL_RESPONSE_TRANSFER_ERRORS[$errno]);
    }
    private static function isConnectTimeoutError(string $error) : bool
    {
        ...
        return Psr7\Utils::caselessContains($error, 'Failed to resolve') && Psr7\Utils::caselessContains($error, 'timeout');
    }
    private static function sanitizeCurlError(#[\SensitiveParameter] string $error, #[\SensitiveParameter] UriInterface $uri, #[\SensitiveParameter] ?string $proxy = null) : string
    {
        if ('' === $error) {
            return $error;
        }
    ...
    private static function redactProxyUserInfo(#[\SensitiveParameter] string $error, #[\SensitiveParameter] ?string $proxy) : string
    {
        if ($proxy === null || $proxy === '') {
            return $error;
        }
     ...
     * @param array<int|string, mixed> $conf
     */
    private function forceFreshConnectionForAuthenticatedProxy(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array &$conf) : void
    {
        $proxy = self::getEffectiveProxy($conf);
        if ($proxy === null || !self::requiresFreshConnectionForAuthenticatedProxy($request, $proxy, $conf) && !$this->isOpaqueShareAnonymousProxyTunnel($request, $proxy, $conf)) {
            return;
        }
    ...
    private function isOpaqueShareAnonymousProxyTunnel(RequestInterface $request, string $proxy, array $conf) : bool
    {
        ...
        if (self::hasCurlProxyAuthorizationHeader($conf) || self::hasCurlProxyTlsCredentials($conf) || self::hasCurlProxyCredentials($conf)) {
            return \false;
        }
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function normalizeCurlHeaderOptions(#[\SensitiveParameter] array &$conf) : void
    {
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function applyProxyConnectHeaderSuppression(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array &$conf) : void
    {
        $proxy = $conf[\CURLOPT_PROXY] ?? null;
        if (!\is_string($proxy) || $proxy === '' || self::isSocksProxy($proxy, $conf) || !self::usesProxyTunnel($request, $conf)) {
            return;
        }
        ...
        $conf[(int) \constant('CURLOPT_SUPPRESS_CONNECT_HEADERS')] = \true;
    }
    /**
     * @param array<int|string, mixed> $conf
     */
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function socksProxySignature(#[\SensitiveParameter] string $proxy, #[\SensitiveParameter] array $conf) : ?string
    {
     ...
     * @return list<string>
     */
    private static function managedProxyAuthorizationHeaderLines(#[\SensitiveParameter] RequestInterface $request) : array
    {
        $headers = [];
     ...
     */
    private static function applyProxyAuthorizationHeaderHandling(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array &$conf, #[\SensitiveParameter] array $managedHeaders) : void
    {
        ...
        $proxy = self::getEffectiveProxy($conf);
        if (CurlVersion::supportsProxyHeaderSeparation() && $proxy !== null && self::isHttpProxyForConnectionReuse($proxy, $conf) && self::usesProxyTunnel($request, $conf)) {
            $conf[(int) \constant('CURLOPT_HEADEROPT')] = (int) \constant('CURLHEADER_SEPARATE');
        }
    }
    /**
     ...
     */
    private static function appendCurlProxyHeaders(#[\SensitiveParameter] array &$conf, #[\SensitiveParameter] array $headers) : void
    {
     ...
     * @param array<int|string, mixed> $conf
     */
    private static function proxyTunnelSignature(#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $conf) : ?string
    {
        $proxy = self::getEffectiveProxy($conf);
        if ($proxy === null) {
            return null;
     ...
     */
    private function getDefaultConf(#[\SensitiveParameter] EasyHandle $easy) : array
    {
        $uri = $easy->request->getUri();
        ...
        $conf = ['_headers' => $easy->request->getHeaders(), \CURLOPT_CUSTOMREQUEST => $easy->request->getMethod(), \CURLOPT_URL => (string) $uri->withFragment(''), \CURLOPT_RETURNTRANSFER => \false, \CURLOPT_HEADER => \false, \CURLOPT_CONNECTTIMEOUT_MS => self::DEFAULT_CONNECT_TIMEOUT_MS];
        ...
        if ('3' === $version || '3.0' === $version) {
            if (!\defined('DeliciousBrains\\WP_Offload_Media\\Gcp\\CURL_HTTP_VERSION_3')) {
            ...
            if (\in_array($multiplex, [Multiplexing::REQUIRE_EAGER, Multiplexing::REQUIRE_WAIT], \true)) {
                ...
                $conf[\CURLOPT_HTTP_VERSION] = $proxy->hasProxy() ? CurlVersion::supportsHttp2() ? \CURL_HTTP_VERSION_2_0 : \CURL_HTTP_VERSION_1_1 : (int) \constant('CURL_HTTP_VERSION_3');
            }
        ...
        if (\in_array($multiplex, [Multiplexing::WAIT, Multiplexing::REQUIRE_WAIT], \true) && CurlVersion::supportsMultiplex() && ($resolvedVersion === \CURL_HTTP_VERSION_2_0 || \defined('CURL_HTTP_VERSION_2_PRIOR_KNOWLEDGE') && $resolvedVersion === (int) \constant('CURL_HTTP_VERSION_2_PRIOR_KNOWLEDGE') || \defined('DeliciousBrains\\WP_Offload_Media\\Gcp\\CURL_HTTP_VERSION_3') && $resolvedVersion === (int) \constant('CURL_HTTP_VERSION_3') || \defined('DeliciousBrains\\WP_Offload_Media\\Gcp\\CURL_HTTP_VERSION_3ONLY') && $resolvedVersion === (int) \constant('CURL_HTTP_VERSION_3ONLY'))) {
    ...
    private function applyMethod(#[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array &$conf, RequestFraming $framing) : void
    {
    ...
    private function applyBody(#[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array &$conf, RequestFraming $framing) : void
    {
        ...
        if ($framing->bodySize !== null && $framing->bodySize < 1000000 || !empty($options['_body_as_string'])) {
            ...
            try {
                ...
                $message = $e instanceof TimeoutException ? 'Timed out while rewinding the request body' : ($e->getMessage() !== '' ? $e->getMessage() : 'Failed to rewind the request body');
    ...
    private static function curlInputSizeOption(#[\SensitiveParameter] RequestInterface $request, int $contentLength) : int
    {
        $option = \defined('DeliciousBrains\\WP_Offload_Media\\Gcp\\CURLOPT_INFILESIZE_LARGE') ? (int) \constant('CURLOPT_INFILESIZE_LARGE') : \CURLOPT_INFILESIZE;
    ...
    private function applyHeaders(#[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array &$conf) : void
    {
    ...
    {
        return FnStream::decorate($stream, ['close' => static function () use($stream) : void {
            $stream->detach();
        }]);
    }
    /**
    ...
    private static function requireStreamFactory($factory) : StreamFactoryInterface
    {
        if (!$factory instanceof StreamFactoryInterface) {
            throw new InvalidArgumentException(\sprintf('%s must be an instance of %s', RequestOptions::STREAM_FACTORY, StreamFactoryInterface::class));
        }
        return $factory;
    }
    private function applyHandlerOptions(#[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array &$conf) : void
    {
        $options = $easy->options;
        ...
        $conf[\CURLOPT_WRITEFUNCTION] = static function ($ch, string $write) use($easy, $sink) : int {
            $length = \strlen($write);
            try {
                $newResponseBodyBytes = TransferByteCounter::add($easy->responseBodyBytes, $length, 'Response body exceeds the maximum integer size supported on this platform');
        ...
        if ($progress !== null || $abortsOnBodyReadFailure) {
            ...
            $progressCallback = static function ($resource, $downloadSize, $downloaded, $uploadSize, $uploaded) use($easy, $progress) : int {
                ...
                try {
                    if ($progress(TransferByteCounter::progressValueToInt($downloadSize), TransferByteCounter::progressValueToInt($downloaded), TransferByteCounter::progressValueToInt($uploadSize), TransferByteCounter::progressValueToInt($uploaded))) {
                        $easy->progressAborted = \true;
                        return 1;
                    }
                    return 0;
                } catch (\Throwable $e) {
    ...
    private function applyTlsVersionRange(#[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array &$conf) : void
    {
        $options = $easy->options;
        ...
        if (($isHttp2 || $isHttp3) && $cryptoMethodMax !== null && TlsVersion::ordinal('crypto_method_max', $cryptoMethodMax) < 12) {
            throw new InvalidArgumentException('Invalid crypto_method_max request option: HTTP/2 and HTTP/3 require TLS 1.2 or higher');
        }
        ...
        $sslVersion = $cryptoMethod === null ? \CURL_SSLVERSION_DEFAULT : self::curlMinSslVersion($easy, $cryptoMethod);
        if ($cryptoMethodMax !== null) {
            $sslVersion |= self::curlMaxSslVersion($easy, $cryptoMethodMax);
        }
        $conf[\CURLOPT_SSLVERSION] = $sslVersion;
    }
    private static function curlMinSslVersion(#[\SensitiveParameter] EasyHandle $easy, int $value) : int
    {
    ...
    private static function curlMaxSslVersion(#[\SensitiveParameter] EasyHandle $easy, int $value) : int
    {
        if ($value === \STREAM_CRYPTO_METHOD_TLSv1_0_CLIENT) {
    ...
    private static function requireCurlMaxSslVersion(#[\SensitiveParameter] EasyHandle $easy, string $constant) : int
    {
        if (\defined($constant)) {
            /** @var int */
            return \constant($constant);
        }
        throw new RequestException('Invalid crypto_method_max request option: maximum TLS version control is not supported by your version of cURL', $easy->request);
    }
    /**
     ...
     * @return PromiseInterface<ResponseInterface, mixed>
     */
    private static function retryFailedRewind(callable $handler, #[\SensitiveParameter] EasyHandle $easy, #[\SensitiveParameter] array $ctx) : PromiseInterface
    {
        try {
    ...
    private function createHeaderFn(#[\SensitiveParameter] EasyHandle $easy) : callable
    {
        ...
        return static function ($ch, string $h) use($onHeaders, $easy, &$startingResponse, &$collectingTrailers, $retainTrailers) : int {
            $value = \trim($h, " \n\r\t\x00\v");
            if ($h === "\r\n" || $h === "\n" || $h === "\r" || $h === '') {
