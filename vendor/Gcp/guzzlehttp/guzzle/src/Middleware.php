...
final class Middleware
{
     ...
     */
    public static function auth(bool $reuseChallenges = \true) : callable
    {
    ...
    public static function cookies() : callable
    {
        return static function (callable $handler) : callable {
            return static function (#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options) use($handler) : PromiseInterface {
                if (empty($options['cookies'])) {
                    return $handler($request, $options);
                } elseif (!$options['cookies'] instanceof CookieJarInterface) {
                    throw new \InvalidArgumentException('DeliciousBrains\\WP_Offload_Media\\Gcp\\cookies must be an instance of GuzzleHttp\\Cookie\\CookieJarInterface');
                }
                $cookieJar = $options['cookies'];
                $request = $cookieJar->withCookieHeader($request);
                return $handler($request, $options)->then(static function (#[\SensitiveParameter] ResponseInterface $response) use($cookieJar, $request) : ResponseInterface {
    ...
    public static function httpErrors(?BodySummarizerInterface $bodySummarizer = null) : callable
    {
        return static function (callable $handler) use($bodySummarizer) : callable {
            return static function (#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options) use($handler, $bodySummarizer) : PromiseInterface {
                if (empty($options['http_errors'])) {
                    return $handler($request, $options);
                }
                return $handler($request, $options)->then(static function (#[\SensitiveParameter] ResponseInterface $response) use($request, $bodySummarizer) : ResponseInterface {
    ...
    public static function history(&$container) : callable
    {
        ...
        return static function (callable $handler) use(&$container) : callable {
            return static function (#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options) use($handler, &$container) : PromiseInterface {
                return $handler($request, $options)->then(static function (#[\SensitiveParameter] ResponseInterface $value) use($request, &$container, $options) : ResponseInterface {
                    $container[] = ['request' => $request, 'response' => $value, 'error' => null, 'options' => $options];
                    return $value;
                }, static function (#[\SensitiveParameter] $reason) use($request, &$container, $options) : PromiseInterface {
    ...
    public static function tap(?callable $before = null, ?callable $after = null) : callable
    {
        return static function (callable $handler) use($before, $after) : callable {
            return static function (#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options) use($handler, $before, $after) : PromiseInterface {
                if ($before) {
    ...
    public static function log(LoggerInterface $logger, MessageFormatterInterface $formatter, string $logLevel = 'info') : callable
    {
        return static function (callable $handler) use($logger, $formatter, $logLevel) : callable {
            return static function (#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options = []) use($handler, $logger, $formatter, $logLevel) : PromiseInterface {
                return $handler($request, $options)->then(
                    static function (#[\SensitiveParameter] ResponseInterface $response) use($logger, $request, $formatter, $logLevel) : ResponseInterface {
                     ...
                     */
                    static function (#[\SensitiveParameter] $reason) use($logger, $request, $formatter) : PromiseInterface {
    ...
    public static function mapRequest(callable $fn) : callable
    {
        return static function (callable $handler) use($fn) : callable {
            return static function (#[\SensitiveParameter] RequestInterface $request, #[\SensitiveParameter] array $options) use($handler, $fn) : PromiseInterface {
