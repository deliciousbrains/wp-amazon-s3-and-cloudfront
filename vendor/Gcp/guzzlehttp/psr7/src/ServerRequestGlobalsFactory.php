...
final class ServerRequestGlobalsFactory
{
     ...
     */
    public static function fromArrays(#[\SensitiveParameter] array $server, array $query, array $post, #[\SensitiveParameter] array $cookies, array $files, ?callable $headerProvider = null) : ServerRequestInterface
    {
        ...
        return $serverRequest->withCookieParams($cookies)->withQueryParams($query)->withParsedBody($post)->withUploadedFiles(UploadedFileNormalizer::normalize($files));
    }
    /**
     * @param array<array-key, mixed> $server Typically the $_SERVER superglobal
     */
    public static function getUriFromServerParams(#[\SensitiveParameter] array $server) : UriInterface
    {
        $method = self::getRequestMethodFromServer($server);
     ...
     */
    private static function getAllHeaders(#[\SensitiveParameter] array $server, ?callable $headerProvider) : array
    {
     ...
     * @return array<array-key, string>
     */
    private static function normalizeHeaderValues(#[\SensitiveParameter] array $headers) : array
    {
        $normalized = [];
    ...
    {
        $headers = [];
        $copyServer = ['CONTENT_TYPE' => 'Content-Type', 'CONTENT_LENGTH' => 'Content-Length', 'CONTENT_MD5' => 'Content-Md5'];
        ...
        if (!isset($headers['Authorization'])) {
            if (isset($server['REDIRECT_HTTP_AUTHORIZATION']) && \is_string($server['REDIRECT_HTTP_AUTHORIZATION'])) {
                ...
                $password = isset($server['PHP_AUTH_PW']) && \is_string($server['PHP_AUTH_PW']) ? $server['PHP_AUTH_PW'] : '';
     ...
     * @param array<array-key, mixed> $server
     */
    private static function getAuthorityUriFromServer(#[\SensitiveParameter] array $server) : UriInterface
    {
     ...
     */
    private static function getUriAndRequestTargetFromServer(#[\SensitiveParameter] array $server, string $method) : array
    {
        ...
        if ($requestUri !== null) {
            ...
            if ($connectAuthority !== null) {
                ...
                $uri = self::getUriWithSchemeFromServer($server);
                return [$uri->withHost($host)->withPort($port)->withPath('')->withQuery(''), $requestUri];
            }
     ...
     * @return array{0: UriInterface, 1: string}|null
     */
    private static function getAbsoluteFormUriAndRequestTarget(#[\SensitiveParameter] string $requestUri, ?string $queryString) : ?array
    {
        ...
        $normalizeRequestTarget = !Rfc9112::isValidRequestTarget($requestTarget) || self::hasEmptyPortInAbsoluteFormRequestTarget($requestTarget);
    ...
    private static function removeUserInfoFromAbsoluteFormRequestTarget(string $target) : string
    {
        ...
        return \substr($target, 0, $authorityStart) . \substr($authority, $lastAt + 1) . \substr($target, $authorityEnd);
    }
