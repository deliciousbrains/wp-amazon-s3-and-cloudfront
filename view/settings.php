...
if ( is_wp_error( $buckets ) ) :
	...
	<div class="error">
		<p>
			...
			<?php echo esc_html( $buckets->get_error_message() ); ?>
		</p>
	</div>
	<?php
endif;
...
<tr valign="top">
	<td>
		...
		<?php if ( is_array( $buckets ) ) foreach ( $buckets as $bucket ): ?>
			<option value="<?php echo esc_attr( $bucket['Name'] ); ?>" <?php echo esc_attr( ( $bucket['Name'] == $this->get_setting( 'bucket' ) ) ? 'selected="selected"' : '' ); ?>><?php echo esc_html( $bucket['Name'] ); ?></option>
		<?php endforeach;?>
		<option value="new"><?php _e( 'Create a new bucket...', 'as3cf' ); ?></option>
		</select><br />

		<input type="checkbox" name="virtual-host" value="1" id="virtual-host" <?php echo esc_attr( $this->get_setting( 'virtual-host' ) ? 'checked="checked" ' : '' ); ?> />
		...
		<input type="checkbox" name="expires" value="1" id="expires" <?php echo esc_attr( $this->get_setting( 'expires' ) ? 'checked="checked" ' : '' ); ?> />
...
<tr valign="top">
	<td>
		...
		<label><?php echo esc_html( trailingslashit( $this->get_dynamic_prefix() ) ); ?></label>
	</td>
</tr>
...
<tr valign="top">
	<td>
		...
		<input type="checkbox" name="copy-to-s3" value="1" id="copy-to-s3" <?php echo esc_attr( $this->get_setting( 'copy-to-s3' ) ? 'checked="checked" ' : '' ); ?> />
		<label for="copy-to-s3"> <?php _e( 'Copy files to S3 as they are uploaded to the Media Library', 'as3cf' ); ?></label>
		<br />

		<input type="checkbox" name="serve-from-s3" value="1" id="serve-from-s3" <?php echo esc_attr( $this->get_setting( 'serve-from-s3' ) ? 'checked="checked" ' : '' ); ?> />
		...
		<input type="checkbox" name="remove-local-file" value="1" id="remove-local-file" <?php echo esc_attr( $this->get_setting( 'remove-local-file' ) ? 'checked="checked" ' : '' ); ?> />
		<label for="remove-local-file"> <?php _e( 'Remove uploaded file from local filesystem once it has been copied to S3', 'as3cf' ); ?></label>
		<br />

		<input type="checkbox" name="force-ssl" value="1" id="force-ssl" <?php echo esc_attr( $this->get_setting( 'force-ssl' ) ? 'checked="checked" ' : '' ); ?> />
		<label for="force-ssl"> <?php _e( 'Always serve files over https (SSL)', 'as3cf' ); ?></label>
		<br />

		<input type="checkbox" name="object-versioning" value="1" id="object-versioning" <?php echo esc_attr( $this->get_setting( 'object-versioning' ) ? 'checked="checked" ' : '' ); ?> />
		...
		<input type="checkbox" name="hidpi-images" value="1" id="hidpi-images" <?php echo esc_attr( $this->get_setting( 'hidpi-images' ) ? 'checked="checked" ' : '' ); ?> />
