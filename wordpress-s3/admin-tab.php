...
function print_file_type( $contentType, $file ) {
	if ( preg_match( '!octet-stream!', $contentType ) ) {
		return preg_replace( '!(.*)\.([a-zA-Z0-9]*)$!', '$2', $file );
	} else {
		return preg_replace( '!vnd.!', '', preg_replace( '!/!', ' ', $contentType ) );
	}
}
?>
...
if ( is_array( $prefixes ) ) foreach ( $prefixes as $prefix ):
	...
	$label = ( $path ? preg_replace( "!$path!", "", '/'.$prefix ) : $prefix );
$label = trim( $prefix, '/' );
if ( preg_match( '!/!', $label ) ) $label = substr( $label, strrpos( $label, '/' )+1 );
?>
...
<?php if ( preg_match( "!^image/.*!", $meta[$i]['content-type'] ) ):?>
...
<?php endif;?>
href="<?php echo $url;?>"
class="file <?php echo preg_replace( '!/!', ' ', $meta[$i]['content-type'] );?>"
